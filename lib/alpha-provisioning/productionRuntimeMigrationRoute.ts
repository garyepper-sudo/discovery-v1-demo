...
import { migrateProductionRuntimeToPostgres, RuntimeMigrationOperationError } from "./productionRuntimeMigrationOperation";
...
const authorized = (request: Request): boolean => {
  const expected = process.env.DISCOVERY_RUNTIME_MIGRATION_SECRET, supplied = request.headers.get("authorization");
...
export function createMigrationPostHandler(operation = migrateProductionRuntimeToPostgres) { return async (request: Request): Promise<Response> => {
  ...
  catch (error) { return error instanceof RuntimeMigrationOperationError ? Response.json(error.receipt, { status: 409, headers: { "Cache-Control": "private, no-store, max-age=0", "X-Robots-Tag": "noindex, nofollow" } }) : privateResponse(409, "Runtime migration operation failed."); }
}; }
