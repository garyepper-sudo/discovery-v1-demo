...
export function createParticipantReferenceMeetingCurrentAccessFromEnvironment(){const key=process.env.DISCOVERY_PARTICIPANT_IDENTITY_LOCATOR_KEY;let url:string;try{url=requireDiscoveryDatabaseUrl("application");}catch{return unavailable;}if(!key||key.length<32)return unavailable;return{async authorize(input:{userId:string;organizationId:string;seriesId:string}):Promise<ParticipantReferenceBridgeOutcome>{const verified=await resolveVerifiedConsumerIdentityFromClerk();if(verified.status!=="verified"||verified.identity.consumerId!==input.userId)return"denied";const sql=postgres(url,{max:1});try{return await new ParticipantReferenceMeetingCurrentAccess(new PostgresParticipantReferenceAccessRepository(sql),new ExistingParticipantIdentityResolutionService(new PostgresAlphaAccessRecordRepository(sql,undefined,key))).authorize({verifiedLocator:verified,organizationId:input.organizationId,meetingSeriesId:input.seriesId});}catch{return"unavailable";}finally{await sql.end({timeout:1});}}};}
