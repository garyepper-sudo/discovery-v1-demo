...
export async function parseFirstUnderstandingForm(data: FormData): Promise<FirstUnderstandingInput> {
  ...
  if (files.length < 1 || files.length > 5 || purposes.length !== files.length) throw new Error("Select 1–5 files and describe each source’s purpose.");
  let total = 0;
  for (const file of files) {
    ...
    total += file.size;
    if (total > 5 * 1024 * 1024) throw new Error("The total upload must be at most 5 MB.");
  ...
  for (const [index, file] of files.entries()) {
    ...
    if (bytes.byteLength !== file.size) throw new Error("The upload changed while being read.");
    const { normalizedText } = decodeAndNormalizeSourceContent(bytes);
    if (!normalizedText.trim()) throw new Error("A source cannot be empty.");
  ...
  return { organizationDisplayName: text(data.get("organizationDisplayName")), understandingPurpose: text(data.get("understandingPurpose")), primaryQuestion: text(data.get("primaryQuestion")), meetingTitle: text(data.get("meetingTitle")), cadence, sources: sources.sort((a,b) => a.name.localeCompare(b.name) || a.exactDigest.localeCompare(b.exactDigest)) };
}
