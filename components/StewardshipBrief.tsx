...
export function StewardshipBrief({
  ...
  const takeaway =
    belief?.belief ??
    report?.brief?.[0] ??
    "Discovery has formed a current best understanding from the evidence.";

  const whyItMatters =
    belief?.whyItMatters ??
    report?.brief?.[1] ??
    "This changes which assumptions leadership should test before making the next decision.";

  const action =
    report?.decisionSupport?.nextStep ??
    report?.nextBestEvidence?.[0] ??
    "Collect one independent source that could challenge the current belief.";
  ...
  const onlyOneThing =
    report?.decisionSupport?.suggestedMeeting ??
  ...
  const confidence = belief?.confidence ?? report?.understandingScore ?? 0;
  const sourceCount = report?.sourceCount ?? report?.evidenceObjects?.length ?? 0;
  ...
  return (
    <div className="aha-prompt stewardship-brief">
      ...
      <p className="brief-intro">
        Discovery has formed a current belief. The next step is to test whether
        leadership should act, investigate, or challenge the model.
      </p>
      ...
      <div className="stewardship-section">
        <span>What would strengthen or weaken this belief</span>
        <ul>
      ...
      <div className="usefulness-check">
        ...
        <div className="feedback-grid">
          <button
            className={cx(
              "secondary-button",
              feedback === "new" && "is-selected"
            )}
            onClick={() => onFeedback("new")}
          >
            New insight
          </button>
          ...
          <button
            className={cx(
              "secondary-button",
              feedback === "confirmed" && "is-selected"
            )}
      ...
      {feedback === "known" && (
        <div className="deeper-mode">
          ...
          <p>
            Great. Discovery should now move one layer deeper instead of
            repeating what leadership already knows.
          </p>
          <ul>
            {nextEvidence.map((item) => (
              <li key={item}>{item}</li>
      ...
      {feedback === "disagree" && (
        <div className="deeper-mode tension-mode">
          ...
          <p>
            Excellent. Upload evidence that challenges this belief so Discovery
            can revise the model instead of defending the current answer.
          </p>
        </div>
      )}
      ...
      {feedback === "confirmed" && (
        <div className="deeper-mode">
          ...
          <p>
            Discovery should now look for the assumption beneath this belief
            that leadership has not fully tested.
          </p>
        </div>
      )}
      ...
      {feedback === "new" && (
        <div className="deeper-mode">
          ...
          <p>
            The next investigation should test whether this belief changes
            decisions, resourcing, or timing.
          </p>
        </div>
      )}

      <button className="primary-button full begin-button" onClick={onBegin}>
