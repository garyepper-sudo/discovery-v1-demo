...
export default function DiscoveryOnboardingExperience({
  ...
  const [onboardingRequestId] = useState(() => crypto.randomUUID());
  const onboardingTestEnabled =
    process.env.NEXT_PUBLIC_DISCOVERY_ONBOARDING_TEST_ENABLED === "true";
  ...
  async function runInvestigation(
    ...
    try {
      const response = await fetch(
        "/api/discovery-lab",
        {
          ...
          body: JSON.stringify({
            organizationId:
              organizationId ??
              (onboardingTestEnabled ? null : `org_${onboardingRequestId}`),
            onboardingRequestId,
            company,
            website,
            industry,
      ...
      if (!data.organizationId) {
        throw new Error("The API did not return an organization id.");
      }
