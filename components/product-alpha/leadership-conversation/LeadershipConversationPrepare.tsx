...
import {projectExecutiveTalkingPoints,projectSavedTalkingPointAdditions,type MeetingExecutiveContinuityV1,type WorkflowOnlyTalkingPointV1} from "../../../product/integration/meetingExecutiveContentProjection";
type ExecutivePrepareViewV1=ChiefFirstPrepareViewV1&{priorCycle:ChiefFirstPrepareViewV1["priorCycle"]&{items?:MeetingExecutiveContinuityV1[];workflowDetails?:string[];workflowOnlyTalkingPoints?:WorkflowOnlyTalkingPointV1[]}};
...
const AgendaPreview=({pack}:{pack:ChiefMeetingPackViewV1})=><div className={styles.artifactPreview}><h4>Purpose</h4><p>{pack.purpose}</p>{pack.desiredOutcomes.length>0&&<><h4>Desired outcomes</h4><List items={pack.desiredOutcomes}/></>}<h4>Agenda</h4><ul>{pack.agendaItems.map(item=><li key={item.itemId}><span className={styles.epistemic}>{item.reason}</span><strong>{item.topic}</strong><br/>{item.desiredOutcome}</li>)}</ul><h4>Closing check</h4><List items={pack.closingCheck}/></div>;
type ProjectedMeetingPackViewV1=ChiefMeetingPackViewV1&{workflowOnlyTalkingPointIndexes?:number[]};
const TalkingPointsPreview=({pack}:{pack:ProjectedMeetingPackViewV1})=>{const talkingPoints=projectExecutiveTalkingPoints(pack.talkingPoints,(pack.workflowOnlyTalkingPointIndexes??[]).map((itemIndex,index)=>({itemId:`presentation:${index}`,itemIndex,originKind:"reviewed-closure",semanticKind:"workflow-status"}))),commitments=talkingPoints.doNotForget,savedAdditions=projectSavedTalkingPointAdditions(pack.generatedTalkingPointsText,pack.talkingPointsText);return <div className={styles.artifactPreview}>{talkingPoints.questionsToAsk.length>0&&<><h4>Questions to ask</h4><List items={talkingPoints.questionsToAsk}/></>}{talkingPoints.currentLeaning.length>0&&<><h4>Tentative view</h4><List items={talkingPoints.currentLeaning}/></>}{talkingPoints.watchOuts.length>0&&<><h4>Watch-outs</h4><List items={talkingPoints.watchOuts}/></>}{commitments.length>0&&<><h4>Commitments to remember</h4><List items={commitments}/></>}{talkingPoints.privateContext.length>0&&<><h4>My private context</h4><List items={talkingPoints.privateContext}/></>}{savedAdditions.length>0&&<><h4>My saved edits</h4><List items={savedAdditions}/></>}</div>};
const projectPack=(pack:ChiefMeetingPackViewV1|null,workflowOnly:WorkflowOnlyTalkingPointV1[]):ProjectedMeetingPackViewV1|null=>pack?{...pack,workflowOnlyTalkingPointIndexes:workflowOnly.map(item=>item.itemIndex)}:null;
...
export function MeetingPackPanel({initialPack,hasPrior=false,onPackChange,seriesAddress,editable=true,workflowOnlyTalkingPoints=[]}:{initialPack:ChiefMeetingPackViewV1|null;hasPrior?:boolean;onPackChange?:(exists:boolean,stale:boolean)=>void;seriesAddress?:string;editable?:boolean;workflowOnlyTalkingPoints?:WorkflowOnlyTalkingPointV1[]}){
  const[pack,setPack]=useState(()=>projectPack(initialPack,workflowOnlyTalkingPoints)),[note,setNote]=useState(""),[intent,setIntent]=useState<MeetingPackPrivateNoteIntentV1>("keep-private"),[agenda,setAgenda]=useState(initialPack?.agendaText??""),[talking,setTalking]=useState(initialPack?.talkingPointsText??""),[editingAgenda,setEditingAgenda]=useState(false),[editingTalking,setEditingTalking]=useState(false),[message,setMessage]=useState<string|null>(null),[pending,start]=useTransition();
  ...
  const update=(next:ChiefMeetingPackViewV1|null,preserveEdits=false)=>{setPack(projectPack(next,workflowOnlyTalkingPoints));onPackChange?.(Boolean(next),Boolean(next?.potentiallyOutOfDate));const buffers=reconcileMeetingPackDraftBuffers({agendaText:agenda,talkingPointsText:talking},next,preserveEdits);setAgenda(buffers.agendaText);setTalking(buffers.talkingPointsText);};
...
function SourceScopedAnalysisPanel({initialMeetingPack,hasPrior,meetingPackUnavailable,onPackChange,seriesAddress,editable,workflowOnlyTalkingPoints}:{initialMeetingPack:ChiefMeetingPackViewV1|null;hasPrior:boolean;meetingPackUnavailable:boolean;onPackChange?:(exists:boolean,stale:boolean)=>void;seriesAddress?:string;editable:boolean;workflowOnlyTalkingPoints:WorkflowOnlyTalkingPointV1[]}){
  ...
  return <>{meetingPackUnavailable?<section className={styles.meetingPack}><h3>Meeting pack unavailable</h3><p role="status">Discovery cannot safely reconstruct this meeting pack with your current access. No private content was shown or changed.</p></section>:<MeetingPackPanel initialPack={initialMeetingPack} hasPrior={hasPrior} onPackChange={onPackChange} seriesAddress={seriesAddress} editable={editable} workflowOnlyTalkingPoints={workflowOnlyTalkingPoints}/>}<section className={styles.supportingAnalysis} aria-label="Working analysis"><p>Analysis is supporting work, not a separate job and not your settled view.</p>{state.status==="idle"?<button type="button" onClick={generate}>Show detailed working analysis</button>:<SourceScopedAnalysisPanelView state={pending&&state.status!=="success"?{status:"pending"}:state}/>}</section></>;
}

export function LeadershipConversationPrepare({ prepare, valueLayer, initialMeetingPack=null, meetingPackUnavailable=false, relevantElsewhere=[], onProgressiveDisclosure,onPackChange,seriesAddress,editable=true }: { prepare: ExecutivePrepareViewV1; valueLayer?: ChiefOfStaffValueLayerV1; initialMeetingPack?:ChiefMeetingPackViewV1|null; meetingPackUnavailable?:boolean; relevantElsewhere?:readonly CrossMeetingRelevanceItemV1[]; onProgressiveDisclosure?:(category:"questions-tensions"|"reasoning-provenance")=>void;onPackChange?:(exists:boolean,stale:boolean)=>void;seriesAddress?:string;editable?:boolean }) {
  if (!valueLayer) return <section aria-labelledby="first-prepare-heading">
    ...
    </details><SourceScopedAnalysisPanel initialMeetingPack={initialMeetingPack} hasPrior={prepare.priorCycle.status==="completed"} meetingPackUnavailable={meetingPackUnavailable} onPackChange={onPackChange} seriesAddress={seriesAddress} editable={editable} workflowOnlyTalkingPoints={prepare.priorCycle.workflowOnlyTalkingPoints??[]}/>
  </section>;
  ...
  return <section aria-labelledby="first-prepare-heading" className={styles.valueLayer}>
    ...
    <div className={styles.orientationGrid}><section><h3>Last time</h3>{prepare.priorCycle.status==="none"?<p>No prior reviewed meeting yet.</p>:prepare.priorCycle.items?.length?<>{prepare.priorCycle.items.map(item=><div key={`${item.label}:${item.text}`} className={styles.continuityItem}><h4>{item.label}</h4><p>{item.text}</p></div>)}{Boolean(prepare.priorCycle.workflowDetails?.length)&&<details><summary>Workflow details</summary><List items={prepare.priorCycle.workflowDetails??[]}/></details>}</>:<p>{prepare.priorCycle.message}</p>}</section><section><h3>Since last time</h3>{prepare.priorCycle.status === "none" ? <p>Change comparison will begin after this meeting cycle is completed.</p> : <ValueList items={valueLayer.changed} empty="No material change is supported." />}</section><section className={styles.priority}><h3>What matters now</h3><ValueList items={valueLayer.attention} empty="No supported priority is available yet." /></section></div>
    <section className={styles.relevantElsewhere} aria-labelledby="relevant-elsewhere-heading"><h3 id="relevant-elsewhere-heading">Relevant elsewhere</h3>{relevantElsewhere.length===0?<p>No reviewed cross-meeting developments are currently relevant.</p>:<div className={styles.relevantElsewhereList}>{relevantElsewhere.map(item=><article className={styles.relevantElsewhereCard} key={`${item.originOccurrenceId}:${item.originArtifactId}`}><p className={styles.badge}>Reviewed · {item.originMeetingTitle}</p><p>{item.safeSummary}</p><details><summary>Why this is relevant</summary><p>{item.safeWhyRelevant}</p><p className={styles.quiet}>Evidence references: {item.citationRefs.length}</p></details></article>)}</div>}</section>
    <SourceScopedAnalysisPanel initialMeetingPack={initialMeetingPack} hasPrior={prepare.priorCycle.status==="completed"} meetingPackUnavailable={meetingPackUnavailable} onPackChange={onPackChange} seriesAddress={seriesAddress} editable={editable} workflowOnlyTalkingPoints={prepare.priorCycle.workflowOnlyTalkingPoints??[]}/>
