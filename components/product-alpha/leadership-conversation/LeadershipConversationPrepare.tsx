...
const TalkingPointsPreview=({pack}:{pack:ChiefMeetingPackViewV1})=><div className={styles.artifactPreview}>{pack.talkingPoints.questionsToAsk.length>0&&<><h4>Questions to ask</h4><List items={pack.talkingPoints.questionsToAsk}/></>}{pack.talkingPoints.currentLeaning.length>0&&<><h4>Tentative view</h4><List items={pack.talkingPoints.currentLeaning}/></>}{pack.talkingPoints.watchOuts.length>0&&<><h4>Watch-outs</h4><List items={pack.talkingPoints.watchOuts}/></>}{pack.talkingPoints.doNotForget.length>0&&<><h4>Do not forget</h4><List items={pack.talkingPoints.doNotForget}/></>}{pack.talkingPoints.privateContext.length>0&&<><h4>My private context</h4><List items={pack.talkingPoints.privateContext}/></>}</div>;
...
export function MeetingPackPanel({initialPack,hasPrior=false,onPackChange,seriesAddress}:{initialPack:ChiefMeetingPackViewV1|null;hasPrior?:boolean;onPackChange?:(exists:boolean,stale:boolean)=>void;seriesAddress?:string}){
  ...
  const update=(next:ChiefMeetingPackViewV1|null,preserveEdits=false)=>{setPack(next);onPackChange?.(Boolean(next),Boolean(next?.potentiallyOutOfDate));const buffers=reconcileMeetingPackDraftBuffers({agendaText:agenda,talkingPointsText:talking},next,preserveEdits);setAgenda(buffers.agendaText);setTalking(buffers.talkingPointsText);};
...
function SourceScopedAnalysisPanel({initialMeetingPack,hasPrior,meetingPackUnavailable,onPackChange,seriesAddress}:{initialMeetingPack:ChiefMeetingPackViewV1|null;hasPrior:boolean;meetingPackUnavailable:boolean;onPackChange?:(exists:boolean,stale:boolean)=>void;seriesAddress?:string}){
