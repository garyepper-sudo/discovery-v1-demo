...
- Every role uses `readScopedOrganizationalProductProjection` and
  `projectScopedDecisionCalibration`.
- Authorized projections perform one bounded repository read; denied and
  cross-organization projections perform zero.
...
Historical Checkpoint L1 is now canonically closed across all four actual-owner relationship kinds, five endpoint-specific access transitions, organization-local addressability, X/Y record isolation, shared-root CAS, and five-world fresh-process A/B/C/D reconstruction. Relationships remain lineage facts and never grant access; list/detail authorization precedes protected loading. Retained Runtime and paused/original candidates remain unchanged. The exact next roadmap task must be selected from the current canonical roadmap; do not begin authoritative Decision linkage, expected Outcome/signal, supersession, ordinary frontend history delivery, universal history, or Architecture Compression without separate authorization.
