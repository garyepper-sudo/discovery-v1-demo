...
| Understanding Recommendation | Take the most useful bounded next step to improve understanding. | Exact current Unknown, canonical investigation opportunity, and authorized source availability. | Record proposal-specific authorization and target the Unknown; external execution remains separate. | Product-facing Understanding Recommendation projected from the ranked Phase 2C proposal, with expected value, cost, purpose, and secondary effects. | Ties remain ties; unsupported work returns no-safe-operation. | ProductQuestion history owns only the existing operation receipt; the Recommendation projection creates no duplicate persistence. | read-only projection, purpose stability, ranking, authorization, replay, conflict, reload, no downstream automation. | Authorize one proposal and confirm the Unknown is targeted without executing external work. | one receipt; duplicate Recommendation store 0; external action 0. |
...
`Question → Google folder search → Answer or targeted abstention → Confidence where eligible → durable Unknown → Understanding Recommendation over the highest-value improvement proposal → explicit authorization → durable receipt → Unknown targeted → separately governed evidence acquisition → revised Answer or explicit no-change → eligible Objective Recommendation where objective/context exist → Decision draft → committed Decision → Outcome review → organizational learning → model-state update → New Insight or qualified abstention → Runtime reload → full ancestry preserved`
