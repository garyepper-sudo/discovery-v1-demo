...
| Improve confidence | Take the most useful bounded next step. | Exact current Unknown, canonical investigation opportunity, and authorized source availability. | Record proposal-specific authorization and target the Unknown; external execution remains separate. | Ranked bounded proposal, expected value and cost, durable authorization receipt. | Ties remain ties; unsupported work returns no-safe-operation. | ProductQuestion history owns the receipt; authorization references the exact Question, Unknown, proposal, action, and actor. | read-only generation, ranking, authorization, replay, conflict, reload, no downstream automation. | Authorize one proposal and confirm the Unknown is targeted without executing external work. | one receipt; duplicate authorization 0; external action 0. |
...
`Question → Google folder search → Answer or targeted abstention → Confidence where eligible → durable Unknown → highest-value improvement proposal → explicit authorization → durable receipt → Unknown targeted → separately governed evidence acquisition → revised Answer or explicit no-change → Recommendation → Decision draft → committed Decision → Outcome review → organizational learning → model-state update → New Insight or qualified abstention → Runtime reload → full ancestry preserved`
