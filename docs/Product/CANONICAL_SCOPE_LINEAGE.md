...
This contract adds no database migration. Northstar reconstruction now supplies the exact topology and source-binding package to `runDiscoveryV3`, the canonical Evidence-admission producer. `evolveOrganizationRuntime` cumulatively retains immutable attribution and deterministic derived lineage from exact support ancestry. Existing callers without structured lineage retain the explicit legacy path. The reconstructed Runtime proves lineage production, but the live adapter still supplies no scoped Product candidates; Product-input population remains a separate bounded gap.
