...
## DEC-PROD-063 — Multi-Meeting Shell is an authorized application projection

Discovery may enumerate several recurring Leadership Conversation series through one read-only, current-access-scoped directory and one parameterized Meeting Home. The directory is not a truth owner and reads metadata only. Opaque route addresses are resolved server-side to existing Product Question, series, occurrence, lifecycle, Prepared Work, Meeting Pack, and private-state owners before rendering or every explicit operation. The legacy Leadership Conversation route remains a thin authorized redirect. Cross-meeting relevance, meeting profiles, calendar integration, Universal Ask, and multi-user alignment/divergence remain deferred.
...
## DEC-PROD-068 — Explicit meeting-scoped participant perspectives
...
The browser supplies only one to four files, one purpose per file, and explicit intent. Participant, organization, Question, occurrence, current scope, and current publication are resolved from the authenticated request and opaque Meeting Home. Full-batch validation precedes the first write, exact replay converges, lawful partial source admission is retained for recovery, Production remains unavailable, and protected bodies remain in Founder Local Alpha external storage. Adding governed context neither completes Organizational Understanding nor builds a Meeting Pack. Founder Local Alpha filesystem persistence remains non-Production; the next deliberate milestone is the first five-source Meeting Pack with source-current-access proof.
