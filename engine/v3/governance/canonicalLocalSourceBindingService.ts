...
import { readOrganizationRuntimeForOperation, RuntimeStorageConflictError, type OrganizationRuntimeRepository, type RuntimeStorageOperationMetadata } from "../runtime/organizationRuntimeRepository";
import {
...
export class CanonicalLocalSourceBindingService {
  ...
  private validateCommon(request:CommonRequest, failureOperation:CanonicalSourceBindingOperation, allowExpectedRuntimeRevision=false):void{
    const allowed=new Set(["contractVersion","organizationId","productQuestionId","sourceType","purposeRef","normalizedContentDigest","requestedScopeAssertions","sensitivity","authorization","recordedAt","recordedByActorRef","idempotencyKey","operation","resolvedAt","availability",...(allowExpectedRuntimeRevision?["expectedRuntimeRevision"]:[])]);
  ...
  private async mutate(request:RegisterCanonicalLocalSourceBindingRequestV1|ReviseCanonicalSourceBindingAvailabilityRequestV1,mode:"register"|"revise"):Promise<CanonicalLocalSourceBindingMutationReceiptV1>{
    ...
    const expectedRuntimeRevision=mode==="revise"?(request as ReviseCanonicalSourceBindingAvailabilityRequestV1).expectedRuntimeRevision:undefined;this.validateCommon(request,failureOperation,mode==="revise");if(!timestamp(request.recordedAt)||request.recordedAt!==this.clock.now()||!exact(request.recordedByActorRef)||!exact(request.idempotencyKey)||(mode==="revise"&&!exact(expectedRuntimeRevision!)))sourceBindingFailure(failureOperation,"SOURCE_BINDING_INVALID_SOURCE_IDENTITY");
    ...
    let loaded:Awaited<ReturnType<OrganizationRuntimeRepository["read"]>>|undefined;try{loaded=await readOrganizationRuntimeForOperation(this.runtimeRepository,request.organizationId,{...request.operation,writerClass:"CanonicalLocalSourceBindingService"});}catch{sourceBindingFailure(failureOperation,"SOURCE_BINDING_PERSISTENCE_FAILED",false,"unknown");}if(!loaded||loaded.runtime.metadata.organizationId!==request.organizationId)sourceBindingFailure(failureOperation,"SOURCE_BINDING_NOT_FOUND");const stored=loaded!;
