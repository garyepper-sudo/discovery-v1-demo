...
async function worker(role:string,root:string){const conditionCount=process.execArgv.filter(argument=>argument===REACT_SERVER_CONDITION).length;assert.equal(conditionCount,1);const repository=new FilesystemOrganizationRuntimeRepository(root);
  if(role==="A"){
    ...
    console.log(JSON.stringify({role,result:"initialized",reactServerConditionActive:true,reactServerConditionCount:conditionCount}));return;
  }
  ...
  console.log(JSON.stringify({role,result:"PASS",reactServerConditionActive:true,reactServerConditionCount:conditionCount,operationId:result.contributionResult.contributionOperationId,resultDigest:result.contributionResult.canonicalResultDigest,canonicalUnderstandingChange:result.contributionResult.canonicalUnderstandingChange,disposition:result.contributionResult.operationDisposition,cognitionDisposition:result.contributionResult.cognitionDisposition,admissions:result.contributionResult.admissions.map(item=>({evidenceId:item.canonicalEvidenceId,admissionId:item.canonicalAdmissionId,attributionId:item.attributionId})),operationEvents,attributionCount:stored!.runtime.memory.canonicalScopeLineageIndex?.evidenceAttributions.length??0}));
}

function execute(role:string,root:string){const childArguments=[REACT_SERVER_CONDITION,"--import","tsx",new URL(import.meta.url).pathname,"--worker",role,root];assert.equal(childArguments.filter(argument=>argument===REACT_SERVER_CONDITION).length,1);const child=spawnSync(process.execPath,childArguments,{shell:false,encoding:"utf8",timeout:30000,maxBuffer:128*1024,env:{NODE_ENV:"test",PATH:process.env.PATH??"",NODE_PATH:process.env.NODE_PATH??""}});assert.equal(child.status,0,`role ${role}: ${child.stderr.slice(0,500)}`);const lines=child.stdout.trim().split("\n").filter(Boolean);return JSON.parse(lines.at(-1)!);}

async function parent(){let checks=0;const outputs=[];for(let run=0;run<2;run+=1){const root=await mkdtemp(path.join(tmpdir(),"discovery-operation-result-replay-"));try{const a=execute("A",root);const b=execute("B",root);const c=execute("C",root);const d=execute("D",root);assert.equal(a.result,"initialized");checks+=1;assert.equal(a.reactServerConditionActive,true);assert.equal(a.reactServerConditionCount,1);checks+=2;assert.equal(b.reactServerConditionActive,true);assert.equal(b.reactServerConditionCount,1);checks+=2;assert.equal(b.admissions.length,2);checks+=1;assert.equal(c.disposition,"idempotent-replay");checks+=1;assert.equal(c.cognitionDisposition,"exact-operation-replay");checks+=1;assert.equal(c.operationId,b.operationId);checks+=1;assert.equal(c.resultDigest,b.resultDigest);checks+=1;assert.deepEqual(c.canonicalUnderstandingChange,b.canonicalUnderstandingChange);checks+=1;assert.deepEqual(c.admissions,b.admissions);checks+=1;assert.equal(c.operationEvents,1);checks+=1;assert.equal(d.cognitionDisposition,"no-new-canonical-input");checks+=1;assert.equal(d.canonicalUnderstandingChange.status,"available");checks+=1;assert.equal(d.canonicalUnderstandingChange.result.disposition,"unchanged");checks+=1;assert.equal(d.operationEvents,2);checks+=1;assert.equal(d.attributionCount,b.attributionCount);checks+=1;outputs.push({operationId:b.operationId,resultDigest:b.resultDigest,canonicalUnderstandingChange:b.canonicalUnderstandingChange,admissions:b.admissions,duplicateAdmissions:d.admissions,duplicateChange:d.canonicalUnderstandingChange});}finally{await rm(root,{recursive:true,force:true});}}
  assert.deepEqual(outputs[0],outputs[1]);checks+=1;console.log(JSON.stringify({result:"PASS",checks,processes:8,runs:2,identical:true,processAReactServerConditionCount:1,processBReactServerConditionCount:1,temporaryRootsRemoved:true,externalActivity:{network:0,connector:0,drive:0,production:0}}));}
