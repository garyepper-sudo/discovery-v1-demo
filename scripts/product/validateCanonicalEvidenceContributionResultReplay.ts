...
function adapter(repository:FilesystemOrganizationRuntimeRepository){return new CanonicalProductWorkspaceAdapter({runtimeRepository:repository,authorize:async({userId,organizationId})=>userId===USER&&organizationId===ORG,investigate:async({runtime,question,contribution})=>{
  ...
  const result=runDiscoveryV3(input,{organizationId:ORG,effectiveAt:AT,topologyRevisions:[topology],sourceBindingRevisions:[sourceBinding],existingEvidenceAttributions:runtime.memory.canonicalScopeLineageIndex?.evidenceAttributions??[]});
...
async function worker(role:string,root:string){const repository=new FilesystemOrganizationRuntimeRepository(root);
  ...
  const service=adapter(repository);const result=await service.contributeEvidenceWithCanonicalResult({userId:USER,organizationId:ORG,questionId:QUESTION_ID,contribution:CONTRIBUTION,operation:{requestId:role==="B"?"contribute":"replay",operatorId:USER}});
  ...
  console.log(JSON.stringify({role,result:"PASS",operationId:result.contributionResult.contributionOperationId,resultDigest:result.contributionResult.canonicalResultDigest,disposition:result.contributionResult.operationDisposition,admissions:result.contributionResult.admissions.map(item=>({evidenceId:item.canonicalEvidenceId,admissionId:item.canonicalAdmissionId,attributionId:item.attributionId})),operationEvents,attributionCount:stored!.runtime.memory.canonicalScopeLineageIndex?.evidenceAttributions.length??0}));
}
...
async function parent(){let checks=0;const outputs=[];for(let run=0;run<2;run+=1){const root=await mkdtemp(path.join(tmpdir(),"discovery-operation-result-replay-"));try{const a=execute("A",root);const b=execute("B",root);const c=execute("C",root);assert.equal(a.result,"initialized");checks+=1;assert.equal(b.admissions.length,2);checks+=1;assert.equal(c.disposition,"idempotent-replay");checks+=1;assert.equal(c.operationId,b.operationId);checks+=1;assert.equal(c.resultDigest,b.resultDigest);checks+=1;assert.deepEqual(c.admissions,b.admissions);checks+=1;assert.equal(c.operationEvents,1);checks+=1;assert.equal(c.attributionCount,b.attributionCount);checks+=1;outputs.push({operationId:b.operationId,resultDigest:b.resultDigest,admissions:b.admissions});}finally{await rm(root,{recursive:true,force:true});}}
  assert.deepEqual(outputs[0],outputs[1]);checks+=1;console.log(JSON.stringify({result:"PASS",checks,processes:6,runs:2,identical:true,temporaryRootsRemoved:true,externalActivity:{network:0,connector:0,drive:0,production:0}}));}
