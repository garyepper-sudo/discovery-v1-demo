...
import { createCanonicalEvidenceScopeAttribution, createCanonicalScopeLineageIndex, createCanonicalScopeTopology, createCanonicalSourceScopeBinding, readCanonicalScopeLineageTopology, resolveCurrentSourceScopeBinding } from "../../engine/v3/governance/canonicalScopeLineage";
...
import { ProductArtifactCurrentOwnerStateResolver } from "../../product/integration/productArtifactCurrentOwnerStateResolver";
import {
  ...
  type ProductArtifactMaterialLineageSeedV2, type ProductArtifactMaterialLineageSeedV3, type ProductArtifactMaterialLineageV3,
...
async function validateDirectEvidenceHardGates():Promise<number>{
  const organizationId="org-direct",productQuestionId="question-direct",artifactId="prepared:direct",artifactRevision="prepared:direct:1",workflowId="workflow:successor",h=(value:unknown)=>productArtifactBodyDigest(value);
  const binding={sourceBindingId:"binding:direct",bindingRevisionId:"binding-revision:direct",bindingRevisionDigest:h("binding"),sourceContentVersionId:"source-version:direct",exactContentDigest:h("exact"),normalizedContentDigest:h("normalized"),storageIntegrityDigest:h("storage")};
  const provenance={contractVersion:"2" as const,lineageVariant:"direct-canonical-evidence" as const,organizationId,productQuestionId,ownerScopeRef:"scope:direct",sourceWorkflowRef:"workflow:source",artifactWorkflowRef:workflowId,operationEnvelope:{contributionOperationId:"operation:direct",requestFingerprint:h("request"),idempotencyKeyDigest:h("key"),lineageEnvelopeDigest:h("lineage"),operationRecordDigest:h("operation")},materialization:{canonicalOperationId:"operation:direct",instructionDigest:h("instruction"),materializationRecordId:"materialization:direct",materializationRecordDigest:h("materialization"),receiptId:"receipt:direct",receiptDigest:h("receipt")},admissionBatch:{batchDigest:h("batch"),admissionDisposition:"admitted" as const},evidence:[{canonicalEvidenceId:"evidence:direct",canonicalAdmissionId:"admission:direct",attributionId:"attribution:direct",attributionVersion:1,attributionDigest:h("attribution"),sourceBindings:[binding]}]};
  const unsigned:Omit<ProductArtifactMaterialLineageSeedV2,"seedDigest">={contractVersion:"2",lineageVariant:"direct-canonical-evidence",organizationId,semanticOwner:"product-decision-draft",productQuestionId,creationOperationId:"operation:direct",lineagePolicyVersion:"direct-canonical-evidence:v2",sourceBindings:[{sourceBindingId:binding.sourceBindingId,bindingRevisionId:binding.bindingRevisionId}],sourceContentVersions:[{sourceBindingId:binding.sourceBindingId,sourceContentVersionId:binding.sourceContentVersionId,normalizedContentDigest:binding.normalizedContentDigest}],canonicalMaterial:[{canonicalObjectId:"evidence:direct",revisionRef:"attribution:direct",owner:"canonical-evidence-admission"}],canonicalUnderstandingRevision:null,projectionSourceRef:null,scopeDigest:h("scope"),purpose:"current-product-delivery",sensitivity:"standard",directEvidenceProvenance:provenance};
  const seed:ProductArtifactMaterialLineageSeedV2={...unsigned,seedDigest:h(unsigned)},lineage=completeProductArtifactMaterialLineageV1({seed,productWorkflowId:workflowId,artifactType:"product-decision-draft",artifactId,artifactRevision});assert.equal(lineage.contractVersion,"2");
  // Replace the hand-built body ref with a repository-issued one before behavioral checks.
  const root=await mkdtemp(path.join(tmpdir(),"discovery-direct-evidence-access-"));try{const repository=createProductArtifactBodyRepository({root}),staged=await repository.stage({contractVersion:"1",organizationId,semanticOwner:"product-decision-draft",artifactType:"product-decision-draft",artifactId,artifactRevision,schemaRef:"product-decision-draft:v1",bytes:serializeProductArtifactBodyV1({contractVersion:"1",title:"direct",summary:"safe"})});const valid=completeProductArtifactInspectionMetadataV1({organizationId,semanticOwner:"product-decision-draft",artifactType:"product-decision-draft",artifactId,artifactRevision,productQuestionId,productWorkflowId:workflowId,creationEnvelopeDigest:h("creation"),materialReferencesDigest:h("material"),protectedBody:staged.body,ownerStageReceiptDigest:staged.receiptDigest,materialLineage:lineage});validateProductArtifactInspectionMetadataV1(valid);
    const governance=resolveScopedGovernanceContext({organizationId,subjectId:"subject-direct",requestedScope:{organizationId,type:"organization",id:organizationId},operation:"product-artifact:read",purpose:"current-product-delivery",sensitivity:"standard",evaluatedAt:"2026-08-10T12:00:00.000Z",temporal:{mode:"current"},serverResolvedAuthority:[{authorityRef:"authority:direct",policyRef:"policy:direct",organizationId,subjectId:"subject-direct",scope:{organizationId,type:"organization",id:organizationId},operations:["product-artifact:read"],sensitivity:["standard"],relationship:"direct",status:"active",validFrom:"2026-01-01T00:00:00.000Z"}]});
    const request={contractVersion:"1" as const,organizationId,subjectId:"subject-direct",operation:"product-artifact:read" as const,purpose:"current-product-delivery",scopeDigest:h("scope"),sensitivity:"standard" as const,evaluatedAt:"2026-08-10T12:00:00.000Z",metadata:valid,governance},current={contractVersion:"1" as const,organizationId,productQuestionId,sourceGovernanceDigest:h("source"),eligibilityDigest:h("eligibility"),eligibilityDisposition:"eligible" as const,projectionRevision:"runtime:1",projectionDigest:h("projection"),canonicalUnderstandingRevision:null,canonicalChangeResultDigest:"not-applicable" as const,lineagePolicyVersion:"direct-canonical-evidence:v2",accessBasis:{contractVersion:"2" as const,kind:"direct-canonical-evidence" as const,canonicalUnderstandingRevision:null,provenanceBundleDigest:lineage.directEvidenceProvenance.bundleDigest}};
    assert.equal(resolveProductArtifactCurrentAccessV1(request,current).disposition,"eligible");const reloaded=JSON.parse(JSON.stringify(valid)) as typeof valid;validateProductArtifactInspectionMetadataV1(reloaded);assert.equal(resolveProductArtifactCurrentAccessV1({...request,metadata:reloaded},current).disposition,"eligible");
    for(const bad of [{...current,canonicalUnderstandingRevision:"fabricated"},{...current,accessBasis:{...current.accessBasis,provenanceBundleDigest:h("tampered")}}, {...current,accessBasis:{contractVersion:"2" as const,kind:"canonical-understanding" as const,canonicalUnderstandingRevision:"understanding:1"}}])assert.equal(resolveProductArtifactCurrentAccessV1(request,bad as ProductArtifactCurrentOwnerStateV1).disposition,"unavailable");
    const tampered=structuredClone(valid);if(tampered.materialLineage?.contractVersion!=="2")throw new Error("Expected V2 lineage.");tampered.materialLineage.directEvidenceProvenance.materialization.receiptDigest=h("tampered");assert.throws(()=>validateProductArtifactInspectionMetadataV1(tampered),/material lineage/);
    const scope={organizationId,type:"organization" as const,id:organizationId},otherScope={organizationId,type:"team" as const,id:"team:other"},topology=createCanonicalScopeTopology({organizationId,topologyVersion:1,effectiveAt:"2026-01-01T00:00:00.000Z",nodes:[scope,otherScope],relationships:[]});
    const canonicalBinding=createCanonicalSourceScopeBinding({organizationId,bindingVersion:1,source:{sourceId:"source:direct",sourceVersion:"1",normalizedContentDigest:h("normalized-owner")},topology,assertions:[{relationship:"applies-to",scope}],basisRefs:["basis:direct"],effectiveAt:"2026-01-01T00:00:00.000Z",sourceType:"authorized-record",purposeRef:"current-product-delivery",availability:"available"});
    const attribution=createCanonicalEvidenceScopeAttribution({organizationId,attributionVersion:1,evidenceId:"evidence:owner",evidenceAdmissionId:"admission:owner",bindings:[canonicalBinding],topology,effectiveAt:"2026-01-01T00:00:00.000Z"});
    const admission={contractVersion:"1" as const,canonicalEvidenceId:"evidence:owner",canonicalAdmissionId:"admission:owner",attributionId:attribution.attributionId,attributionVersion:attribution.attributionVersion,investigationEvidenceIds:["local:owner"],sourceBindings:[{sourceBindingId:canonicalBinding.bindingId,sourceId:canonicalBinding.source.sourceId,sourceVersion:canonicalBinding.source.sourceVersion,normalizedContentDigest:canonicalBinding.source.normalizedContentDigest}],disposition:"new-canonical-evidence" as const,attributionDigest:attribution.digest},batchUnsigned={contractVersion:"1" as const,organizationId,admissions:[admission],admissionDisposition:"admitted" as const},batch={...batchUnsigned,batchDigest:h(batchUnsigned)};
    const operationUnsigned={kind:"canonical-evidence-contribution-operation" as const,contractVersion:"1" as const,organizationId,questionId:productQuestionId,contributionOperationId:"operation:owner",idempotencyKeyDigest:h("owner-key"),requestFingerprint:h("owner-request"),canonicalAdmissionBatch:batch,lineageEnvelopeDigest:h("owner-lineage"),evidenceAccepted:true,productQuestionRevisionBefore:1,productQuestionRevisionAfter:2,recordedAt:"2026-08-10T11:00:00.000Z"},operation={...operationUnsigned,recordDigest:h(operationUnsigned)};
    const ownerProvenance={contractVersion:"2" as const,lineageVariant:"direct-canonical-evidence" as const,organizationId,productQuestionId,ownerScopeRef:"scope:owner",sourceWorkflowRef:"workflow:owner-source",artifactWorkflowRef:workflowId,operationEnvelope:{contributionOperationId:operation.contributionOperationId,requestFingerprint:operation.requestFingerprint,idempotencyKeyDigest:operation.idempotencyKeyDigest,lineageEnvelopeDigest:operation.lineageEnvelopeDigest,operationRecordDigest:operation.recordDigest},materialization:{canonicalOperationId:operation.contributionOperationId,instructionDigest:h("owner-instruction"),materializationRecordId:"materialization:owner",materializationRecordDigest:h("owner-materialization"),receiptId:"receipt:owner",receiptDigest:h("owner-receipt")},admissionBatch:{batchDigest:batch.batchDigest,admissionDisposition:batch.admissionDisposition},evidence:[{canonicalEvidenceId:admission.canonicalEvidenceId,canonicalAdmissionId:admission.canonicalAdmissionId,attributionId:attribution.attributionId,attributionVersion:attribution.attributionVersion,attributionDigest:attribution.digest,sourceBindings:[{sourceBindingId:canonicalBinding.bindingId,bindingRevisionId:canonicalBinding.bindingId,bindingRevisionDigest:canonicalBinding.digest,sourceContentVersionId:"source-content:owner:v1",exactContentDigest:h("owner-exact"),normalizedContentDigest:canonicalBinding.source.normalizedContentDigest,storageIntegrityDigest:h("owner-storage")}] }]};
    const ownerSeedUnsigned:Omit<ProductArtifactMaterialLineageSeedV2,"seedDigest">={...unsigned,semanticOwner:"product-decision-draft",creationOperationId:operation.contributionOperationId,sourceBindings:[{sourceBindingId:canonicalBinding.bindingId,bindingRevisionId:canonicalBinding.bindingId}],sourceContentVersions:[{sourceBindingId:canonicalBinding.bindingId,sourceContentVersionId:"source-content:owner:v1",normalizedContentDigest:canonicalBinding.source.normalizedContentDigest}],canonicalMaterial:[{canonicalObjectId:admission.canonicalEvidenceId,revisionRef:attribution.attributionId,owner:"canonical-evidence-admission"}],scopeDigest:h(scope),directEvidenceProvenance:ownerProvenance},ownerSeed:ProductArtifactMaterialLineageSeedV2={...ownerSeedUnsigned,seedDigest:h(ownerSeedUnsigned)},ownerLineage=completeProductArtifactMaterialLineageV1({seed:ownerSeed,productWorkflowId:workflowId,artifactType:"product-decision-draft",artifactId,artifactRevision});assert.equal(ownerLineage.contractVersion,"2");
    const {contractVersion:_ownerContract,headerDigest:_ownerHeader,...ownerMetadataInput}=valid,ownerMetadata=completeProductArtifactInspectionMetadataV1({...ownerMetadataInput,materialLineage:ownerLineage}),ownerRequest={...request,metadata:ownerMetadata};let ownerProof=true,exactMetadata=true,bodyReads=0,runtime:any={metadata:{organizationId},memory:{events:[operation],canonicalScopeLineageIndex:createCanonicalScopeLineageIndex({organizationId,topology,sourceBindings:[canonicalBinding],evidenceAttributions:[attribution]})}},stored:any={revision:"runtime:owner:1",runtime};
    const ownerResolver=()=>new ProductArtifactCurrentOwnerStateResolver({runtimeRepository:{read:async()=>({...stored,runtime})},verifyDirectEvidenceOwnerProof:async()=>ownerProof,resolveExactSourceMetadata:async()=>exactMetadata?{sourceContentVersionId:"source-content:owner:v1",exactContentDigest:h("owner-exact"),normalizedContentDigest:canonicalBinding.source.normalizedContentDigest,storageIntegrityDigest:h("owner-storage")}:null});
    const resolveOwner=async()=>ownerResolver().resolve({organizationId,subjectId:ownerRequest.subjectId,purpose:ownerRequest.purpose,sensitivity:ownerRequest.sensitivity,evaluatedAt:ownerRequest.evaluatedAt,governance:ownerRequest.governance,metadata:ownerMetadata}),expectUnavailable=async()=>{const state=await resolveOwner();assert.notEqual(state.eligibilityDisposition,"eligible");assert.equal(resolveProductArtifactCurrentAccessV1(ownerRequest,state).disposition,"unavailable");assert.equal(bodyReads,0);};
    const exactState=await resolveOwner();assert.equal(exactState.eligibilityDisposition,"eligible");assert.equal(resolveProductArtifactCurrentAccessV1(ownerRequest,exactState).disposition,"eligible");assert.equal(bodyReads,0);
    ownerProof=false;await expectUnavailable();ownerProof=true;exactMetadata=false;await expectUnavailable();exactMetadata=true;
    const originalRuntime=structuredClone(runtime);runtime={...originalRuntime,memory:{...originalRuntime.memory,events:[]}};await expectUnavailable();runtime={...originalRuntime,memory:{...originalRuntime.memory,events:[operation,operation]}};await expectUnavailable();runtime={...originalRuntime,memory:{...originalRuntime.memory,canonicalScopeLineageIndex:{...originalRuntime.memory.canonicalScopeLineageIndex,evidenceAttributions:[]}}};await expectUnavailable();runtime={...originalRuntime,memory:{...originalRuntime.memory,canonicalScopeLineageIndex:{...originalRuntime.memory.canonicalScopeLineageIndex,evidenceAttributions:[attribution,attribution]}}};await expectUnavailable();
    const revoked=createCanonicalSourceScopeBinding({organizationId,bindingVersion:2,source:canonicalBinding.source,topology,assertions:[{relationship:"applies-to",scope}],basisRefs:["basis:revoked"],effectiveAt:"2026-08-10T11:30:00.000Z",supersedesBindingId:canonicalBinding.bindingId,sourceType:"authorized-record",purposeRef:"current-product-delivery",availability:"revoked"}),outOfScope=createCanonicalSourceScopeBinding({organizationId,bindingVersion:2,source:canonicalBinding.source,topology,assertions:[{relationship:"applies-to",scope:otherScope}],basisRefs:["basis:other"],effectiveAt:"2026-08-10T11:30:00.000Z",supersedesBindingId:canonicalBinding.bindingId,sourceType:"authorized-record",purposeRef:"current-product-delivery",availability:"available"});
    for(const successor of [revoked,outOfScope]){runtime={...originalRuntime,memory:{...originalRuntime.memory,canonicalScopeLineageIndex:createCanonicalScopeLineageIndex({organizationId,topology,sourceBindings:[canonicalBinding,successor],evidenceAttributions:[attribution]})}};await expectUnavailable();}runtime=originalRuntime;
    console.log("direct-evidence-authorization-hard-gates: PASS (31 behavioral checks; actual owner resolver; generic owner; reload, revocation, ambiguity, and tamper fail closed; zero body reads)");return 31;
...
async function validateLegacyBootstrapPreparedWorkCurrentAccess():Promise<void>{
...
async function validateBootstrapPreparedWorkProofInvariant():Promise<void>{
  const organizationId="bootstrap-proof-org",questionId="bootstrap-proof-question",artifactId="bootstrap-proof-artifact",artifactRevision="1",workflowId="leadership-conversation:bootstrap-proof",h=(value:unknown)=>productArtifactBodyDigest(value),scope={organizationId,type:"organization" as const,id:organizationId},root=await mkdtemp(path.join(tmpdir(),"discovery-bootstrap-proof-"));
  try{
    const repository=createProductArtifactBodyRepository({root}),staged=await repository.stage({contractVersion:"1",organizationId,semanticOwner:"leadership-conversation",artifactType:"prepared-work",artifactId,artifactRevision,schemaRef:"prepared-work:v1",bytes:serializeProductArtifactBodyV1({contractVersion:"1",title:"bootstrap",summary:"safe"})}),preparationScope={...completeRegisteredMeetingPreparationScopeV1({contractVersion:"1",scopeId:"scope:bootstrap",organizationId,questionId,conversationId:"bootstrap-proof",seriesId:"series:bootstrap",sourceVersions:[{sourceBindingId:"binding:bootstrap",sourceContentVersionId:"source:bootstrap",normalizedContentDigest:h("source")}],createdAt:"2026-09-10T00:00:00.000Z",createdByUserId:"founder",idempotencyKeyDigest:h("scope-key"),requestFingerprint:h("scope-request")}),expectedRevision:"workflow:operational-revision"},base={contractVersion:"3" as const,lineageVariant:"initial-understanding-bootstrap" as const,organizationId,semanticOwner:"leadership-conversation" as const,productQuestionId:questionId,creationOperationId:"bootstrap-operation",lineagePolicyVersion:"bootstrap:v1",sourceBindings:[{sourceBindingId:"binding:bootstrap",bindingRevisionId:"binding:bootstrap"}],sourceContentVersions:[...preparationScope.sourceVersions],canonicalMaterial:[],canonicalUnderstandingRevision:null,projectionSourceRef:null,scopeDigest:h(scope),purpose:"current-product-delivery",sensitivity:"standard" as const,bootstrapOperationId:"bootstrap-operation",bootstrapFingerprint:h("bootstrap"),preparationScopeDigest:preparationScope.scopeDigest},makeMetadata=(seedBase:typeof base)=>{const seed={...seedBase,seedDigest:h(seedBase)} as ProductArtifactMaterialLineageSeedV3,lineage=completeProductArtifactMaterialLineageV1({seed,productWorkflowId:workflowId,artifactType:"prepared-work",artifactId,artifactRevision});return completeProductArtifactInspectionMetadataV1({organizationId,semanticOwner:"leadership-conversation",artifactType:"prepared-work",artifactId,artifactRevision,productQuestionId:questionId,productWorkflowId:workflowId,creationEnvelopeDigest:h("create"),materialReferencesDigest:h("material"),protectedBody:staged.body,ownerStageReceiptDigest:staged.receiptDigest,materialLineage:lineage});},metadata=makeMetadata(base),governance=resolveScopedGovernanceContext({organizationId,subjectId:"founder",requestedScope:scope,operation:"product-artifact:read",purpose:"current-product-delivery",sensitivity:"standard",evaluatedAt:"2026-09-10T00:00:00.000Z",temporal:{mode:"current"},serverResolvedAuthority:[{authorityRef:"authority",policyRef:"policy",organizationId,subjectId:"founder",scope,operations:["product-artifact:read"],sensitivity:["standard"],relationship:"direct",status:"active",validFrom:"2026-01-01T00:00:00.000Z"}]}),request={contractVersion:"1" as const,organizationId,subjectId:"founder",operation:"product-artifact:read" as const,purpose:"current-product-delivery",scopeDigest:h(scope),sensitivity:"standard" as const,evaluatedAt:"2026-09-10T00:00:00.000Z",governance},runtime={read:async()=>({runtime:{metadata:{organizationId},memory:{initialUnderstandingBootstrap:{initialProductQuestionId:questionId,bootstrapOperationId:"bootstrap-operation",requestFingerprint:h("bootstrap")}}}} as any)},verify=async({lineage}:{lineage:ProductArtifactMaterialLineageV3})=>{try{assertBootstrapPreparedWorkScopeProofV1({scope:preparationScope,lineage});return true;}catch{return false;}},resolver=new ProductArtifactCurrentOwnerStateResolver({runtimeRepository:runtime,verifyBootstrapPreparedWorkProof:verify}),allowed=await resolver.resolve({...request,metadata});assert.equal(resolveProductArtifactCurrentAccessV1({...request,metadata},allowed).disposition,"eligible");
    for(const malformed of [makeMetadata({...base,preparationScopeDigest:h("wrong-preparation-scope")}),makeMetadata({...base,sourceBindings:[{sourceBindingId:"binding:bootstrap",bindingRevisionId:"binding:foreign"}]}),makeMetadata({...base,sourceContentVersions:[{sourceBindingId:"binding:other",sourceContentVersionId:"source:other",normalizedContentDigest:h("other")}],sourceBindings:[{sourceBindingId:"binding:other",bindingRevisionId:"binding:other"}]})]){const denied=await resolver.resolve({...request,metadata:malformed});assert.equal(resolveProductArtifactCurrentAccessV1({...request,metadata:malformed},denied).disposition,"unavailable");}console.log("bootstrap-prepared-work-proof-invariant: PASS (production scope serialization, reconstruction, and malformed proof rejection before body read)");
  }finally{await rm(root,{recursive:true,force:true});}
}
...
if (
  ...
  Promise.all([
    ...
    validateBootstrapPreparedWorkProofInvariant(),
