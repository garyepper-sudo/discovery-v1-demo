...
async function main() {
  ...
  try {
    ...
    check(meetingHome.includes("priorWorkspace.closureCompletion") && meetingHome.includes("projectMeetingExecutiveContinuity") && meetingHome.includes("const closure = priorWorkspace.closureCompletion") && !meetingHome.includes("approved continuity is reflected here"), "successor orientation renders only concrete owner-issued reviewed continuity");
    ...
    check(prepare.includes("Development synthesis draft — review before use.") && prepare.includes("Draft · not yet shared") && prepare.includes("Private · only you can see this"), "pack-level review notice and separate artifact privacy labels render");
    ...
    check(sandboxMeetingHome.includes("server.workspace") && sandboxMeetingHome.includes("meeting.occurrenceId") && sandboxMeetingHome.indexOf("resolveAuthorizedMeetingAddress") < sandboxMeetingHome.indexOf("server.workspace") && sandboxMeetingHome.indexOf("server.workspace") < sandboxMeetingHome.indexOf("getPersonalRoomSheetPreviewAction(seriesAddress)"), "fresh reload resolves authorized occurrence before personal work");
    check(sandboxMeetingHome.indexOf("resolveAuthorizedMeetingAddress") < sandboxMeetingHome.indexOf("server.workspace") && sandboxMeetingHome.includes("if (!meeting) notFound()"), "unavailable meeting fails closed before workspace read");
    ...
    check(actions.includes("dispositionOccurrence1CarryForwardAction") && actions.includes("resumeOccurrence1CarryForwardRouteAction") && component.includes("Confirm what Discovery should carry forward") && component.includes('review(proposal.proposalId,"accept")') && component.includes('review(proposal.proposalId,"correct")') && component.includes('review(proposal.proposalId,"reject")') && component.includes('review(proposal.proposalId,"needs-information")') && component.includes("Resume recording") && component.includes("reviewedCarryForwardCompletion"), "Reviewed Carry-Forward remains an explicit, resumable human disposition boundary");
    ...
    check(actions.includes("reviewedCarryForwardContext(input.seriesAddress)")&&actions.includes("context.server.reviewCarryForwardAcknowledged({...context.identity,seriesId:context.seriesId")&&actions.includes("context.server.resumeCarryForwardAcknowledged({...context.identity,seriesId:context.seriesId,purposeRef")&&actions.includes("context.server.finalizeReviewedCarryForward({...context.identity,seriesId:context.seriesId})")&&actions.includes("resumeOccurrence1CarryForwardRouteAction")&&actions.includes("resumeOccurrence1CarryForwardCompletionAction(seriesAddress?:string)")&&server.includes("ensureReviewedCarryForwardRoute=async(input:Parameters<CanonicalLeadershipConversationOwnerRouter[\"routeApproved\"]>[0]&{seriesId?:string})")&&server.includes("ensureReviewedCarryForwardCompletion=async(input:{userId:string;organizationId:string;questionId:string;conversationId:string;seriesId?:string})")&&server.includes("ensureReviewedCarryForwardRoute({...input")&&server.includes("operations.completeReviewedCarryForward({...input"),"Founder review actions forward the server-resolved persisted series through disposition, routing, and completion workspace reconstruction");
    ...
    check(sandboxMeetingHome.indexOf("resolveAuthorizedMeetingAddress")<sandboxMeetingHome.indexOf("server.workspace")&&component.includes("telemetryNotice&&<AlphaTelemetryNotice"),"Meeting Home authorization precedes protected Product rendering");
