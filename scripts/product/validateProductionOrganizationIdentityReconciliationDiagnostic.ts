...
async function main(): Promise<void> {
  ...
  const optionalConfiguration = await reconcileProductionOrganizationIdentity({ VERCEL_ENV: "production", NODE_ENV: "production", DISCOVERY_DATABASE_URL: "postgresql://safe/test" } as NodeJS.ProcessEnv, dependencies({ founder: { organization_id: founderId, creation_key: "founder-production-smoke:asterline-software-synthetic-test:v2" } }));
  check(optionalConfiguration.configMatchesLegacyRuntimeMetadata === false && optionalConfiguration.legacyCandidate.status === "ABSENT", "absent legacy configuration remains optional historical evidence and does not block canonical founder reads");
