...
function createAdapter(repository:MemoryRepository,counters:{investigate:number;preflight:number}){return new CanonicalProductWorkspaceAdapter({runtimeRepository:repository,authorize:async({userId,organizationId})=>userId===USER&&organizationId===ORG,preflightCanonicalEvidence:async({runtime,question,contribution})=>{counters.preflight+=1;const value=context(runtime,question,contribution);return resolveCanonicalEvidenceAdmission(value.input,value.lineage);},investigate:async({runtime,question,contribution})=>{counters.investigate+=1;const {input,lineage}=context(runtime,question,contribution);const result=runDiscoveryV3(input,lineage);const original=console.log;let evolved;try{console.log=()=>{};evolved=evolveOrganizationRuntime({runtime,result,input,semanticTime:AT});}finally{console.log=original;}return{runtime:evolved,evidenceAccepted:result.evidence.length>0,canonicalEvidenceAdmissionBatch:result.scopeLineageAdmission!.operationBatch};}});}
