...
async function main(): Promise<void> {
  ...
  check("route requires its dedicated bootstrap secret before Clerk or provisioner use", source.indexOf("if (!authorized(request))") < source.indexOf("const clerkSubject = await founderSubject()"));
  check("secret comparison is constant-time and never serialized", source.includes("timingSafeEqual") && !/DISCOVERY_FOUNDER_BOOTSTRAP_SECRET[^\n]*Response\.json/u.test(source) && !source.includes("console."));
