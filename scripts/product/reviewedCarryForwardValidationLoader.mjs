...
export async function createFounderMeetingHomeComposition(seriesAddress){
  ...
  return{server,request:{consumerId:input.denyAccess?"user_validation_denied":input.userId},meeting:{organizationId:input.organizationId,questionId:input.questionId,occurrenceId:input.conversationId,seriesId:input.seriesId},close:async()=>{state.closed=true;state.closeCalls+=1;if(input.closeFailure)throw new Error("validation-finalization-close-failure");}};
}`)}`;
