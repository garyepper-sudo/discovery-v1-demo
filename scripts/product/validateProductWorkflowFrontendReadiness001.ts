...
async function main() {
  ...
  try {
    ...
    const actionIds = result.workspace.actions.map((action) => action.id); assert.equal(actionIds.length, new Set(actionIds).size); assert.ok(result.workspace.actions.find((action) => action.id === "execute-existing-local-read-only-operation" && !action.enabled && action.blockedReason === "operation-owner-unimplemented"));
    ...
    const canonicalExecutorDeclaration = /export\s+(?:async\s+)?function\s+(?:execute|run)(?:CompareExistingEvidence|InspectExistingEvidence)|export\s+const\s+(?:execute|run)(?:CompareExistingEvidence|InspectExistingEvidence)/;
    const executorDeclarations = executorSearch.flat().filter(({ source }) => canonicalExecutorDeclaration.test(source));
    assert.deepEqual(executorDeclarations.map(({ file }) => file), [], "A canonical local existing-evidence executor now exists; B-O must be reevaluated.");
    const candidateTypesAreNotExecutors = executorDeclarations.length === 0;
    ...
    const classification = "B-O — REAL-OPERATION WORKFLOW REPLAY BLOCKED";
    const replay = { classification, organizationId, questionCreated: true, independentlyReloaded: true, authorizedWorkspaceRead: true, operationRan: false, blocker: "No canonical local read-only operation executor exists. Candidate action types and action-owner references do not constitute an executor.", informationProduced: false, evidenceAdmitted: false, unknownChanged: false, answerChanged: false, understandingChanged: false, learningRecorded: false, connectorCalls: 0, externalActions: 0, productionOperations: 0, temporaryRuntimeRemoved: true };
    const output = { validation: "product-workflow-frontend-readiness-001", result: "PASS-WITH-BLOCK", classification, contractVersion: "2", fixtures: fixtures.length, authorizationChecks: 1, repositoryReads: 1, payloadBytes: result.payloadBytes, disclosedObjectCounts: { stages: result.workspace.stages.length, actions: result.workspace.actions.length }, unavailableCount: result.workspace.unavailableFields.length, withheldCount: result.workspace.withheldFields.length, deterministicSerializationDigest: createHash("sha256").update(serialized).digest("hex"), exactReset: true, unsupportedSerializationRejected: true, candidateTypesAreNotExecutors, executorFilesInspected: executorSearch.flat().length, replay, hashes };
    const report = `# Product Workflow Frontend Readiness 001\n\n**Classification:** ${classification}\n\nThe version-2 frontend boundary, deterministic orientation, semantic action surface, 24-state fixture pack, one-read authorization behavior, and exact reset are validated. The complete GAP-A-005 replay is blocked because the repository contains governed local action types but no canonical operation executor. No execution was simulated.\n\n- Authorization checks: 1\n- Runtime repository reads: 1\n- Payload bytes: ${result.payloadBytes}\n- Fixture states: ${fixtures.length}\n- Connector calls: 0\n- External actions: 0\n- Production operations: 0\n`;
    const gapReport = `# GAP-A-005 Workflow Acceptance Replay\n\n**Classification:** ${classification}\n\nQuestion creation, filesystem persistence, authorization-before-read, independent reload, and frontend projection succeeded for temporary organization \`${organizationId}\`. The replay stopped before operation execution because no existing canonical local read-only executor owns \`compare-existing-evidence\` or \`inspect-existing-evidence\`. Candidate semantics were not treated as executable behavior. No Outcome, Evidence admission, cognitive change, or Learning was fabricated.\n`;
