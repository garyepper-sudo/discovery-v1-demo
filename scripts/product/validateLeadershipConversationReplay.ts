...
async function execute(root: string, role: string, lineageFixtureRoot: string | null, expectedSeedDigest: string | null, ...handoffs: SafeHandoff[]): Promise<WorkerResult> {
  ...
  const { stdout, stderr } = await runFile(process.execPath, args, { cwd: process.cwd(), env: { PATH: process.env.PATH ?? "", NODE_PATH: process.env.NODE_PATH ?? "", NODE_ENV: "test", TZ: "UTC", LANG: "C", TMPDIR: tmpdir(),AR3_OBSERVER_MODE:process.env.AR3_OBSERVER_MODE??"enabled" }, timeout: 30_000, maxBuffer: 128 * 1024, shell: false });
  assert.equal(stderr, "");
