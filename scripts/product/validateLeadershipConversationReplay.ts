...
async function validationComposition(locations: ReturnType<typeof roots>, lineageFixtureRoot?: string,authorityGrants?:readonly ScopedAuthorityGrant[]) {
  ...
  const mode=process.env.AR3_OBSERVER_MODE,observer=mode==="disabled"?new AlphaContentSafeObservabilityOwner():mode==="throwing"?new AlphaContentSafeObservabilityOwner({emit(){throw new Error("contained observer failure");}}):mode==="rejecting"||mode==="malformed"?{observe:async()=>"rejected" as const}:new AlphaContentSafeObservabilityOwner({emit:event=>{observedEvents.push(event);}});activeObserver=observer instanceof AlphaContentSafeObservabilityOwner?observer:undefined;
  return createLeadershipConversationServerCompositionForValidation({ ...locations, lineageFixtureRoot, userId: fixture.actorId, organizationId: fixture.organizationId,observer,authorityGrants });
}

type SafeHandoff = Record<string, unknown> & { handoffDigest: string };
type WorkerResult = { role: string; handoff: SafeHandoff; assertions: string[]; observations?:AlphaContentSafeObservabilityEventV1[] };
...
async function processA(root: string,lineageFixtureRoot?:string): Promise<WorkerResult> {
  ...
  await composition.freeze({ ...identity, idempotencyKey: "process-a-freeze", artifactVersionId: prepared.artifactRevision, privateWorkingContribution:{seriesId:`leadership-conversation-series:${fixture.conversationId}`,occurrenceId:fixture.conversationId,authorizationRevision:NORTHSTAR_PREPARED_LINEAGE.authorizedProjectionRevision,provenanceDigest:NORTHSTAR_PREPARED_LINEAGE.authorizedProjectionDigest,selectedContent:[]} });
  stored = await workflow.read(fixture.organizationId);
  const frozen = stored.store.frozenSnapshotPublications!.at(-1)!;
  const occurrence=await workflow.readOccurrence!({...identity,seriesId:`leadership-conversation-series:${fixture.conversationId}`}),checkpoint=resolveCurrentOccurrenceCheckpointIdentityV1({store:occurrence.store,organizationId:identity.organizationId,questionId:identity.questionId,conversationId:identity.conversationId});
  assert.equal(checkpoint.checkpointId,frozen.artifactId);assert.equal(checkpoint.contributionArtifactIds.length,0);assert.equal(occurrence.store.publicationReceipts?.filter(value=>value.receiptKind==="frozen-checkpoint-publication").length,1);
  ...
  return { role: "prepare-and-freeze", handoff: manifest, assertions: ["context-persisted", "preparation-v1-persisted", "preparation-v2-persisted", "explicit-empty-frozen-snapshot-persisted", "explicit-empty-occurrence-slice-proof-reconstructed"] };
}
...
async function processB(root: string, encodedA: string): Promise<WorkerResult> {
  ...
  await composition.readFrozenPrivateWorkingContribution({ ...identity, snapshotId: String(a.frozenSnapshotId), artifactIds: stored.store.frozenSnapshotPublications!.at(-1)?.privateWorkingContributionRefs?.map(value=>value.artifactId)??[] });
...
async function processC(root: string, lineageFixtureRoot: string, expectedSeedDigest: string, encodedA: string, encodedB: string, ambiguousEvidence=false): Promise<WorkerResult> {
  ...
  let sourceWorkspace=await composition.workspace(identity);assert.ok(sourceWorkspace.canonicalRoutingReceipts.some(value=>value.ownerKind==="evidence"&&value.proposalId===material.proposalId&&value.dispositionReceiptId===material.dispositionReceiptId));stored=await workflow.read(fixture.organizationId);const frozenClosure=resolveCurrentOccurrenceClosureMetadataV1({store:stored.store,organizationId:fixture.organizationId,questionId,conversationId:identity.conversationId});assert.equal(sourceWorkspace.currentPreparedWorkProduct,null);const closureInput={...identity,seriesId:frozenClosure.seriesId,expectedWorkflowRevision:sourceWorkspace.workflowRevision,authorizedProjectionDigest:frozenClosure.authorizedProjectionDigest,candidateAssessmentDigest:null,b11CommunicationDigest:null,personalRoomSheetDigest:frozenClosure.personalRoomSheetDigest,idempotencyKey:"process-c-closure"};if(!sourceWorkspace.closureCompletion){await composition.completeCycle1Closure(closureInput);sourceWorkspace=await composition.workspace(identity);}assert.ok(sourceWorkspace.closureCompletion);assert.equal(sourceWorkspace.closureCompletion.authorizedProjectionDigest,frozenClosure.authorizedProjectionDigest);assert.equal(sourceWorkspace.closureCompletion.personalRoomSheetDigest,frozenClosure.personalRoomSheetDigest);assert.ok(sourceWorkspace.closureCompletion.reviewedProposalIds.includes(material.proposalId)&&sourceWorkspace.closureCompletion.canonicalRoutingLinkIds.some(value=>sourceWorkspace.routingLinks.some(link=>link.routingLinkId===value&&link.proposalId===material.proposalId)));
  const frozenPublication=stored.store.frozenSnapshotPublications!.find(value=>value.artifactId===frozenClosure.checkpointId)!,sourcePublication=stored.store.preparedWorkPublications!.find(value=>value.artifactRevision===frozenPublication.preparedWorkProductVersionId)!;
  const predecessorRequest={contractVersion:"2" as const,organizationId:identity.organizationId,predecessorArtifactOrganizationId:identity.organizationId,productQuestionId:identity.questionId,meetingSeriesId:frozenClosure.seriesId,predecessorOccurrenceId:identity.conversationId,predecessorConversationId:identity.conversationId,predecessorCheckpointId:frozenClosure.checkpointId,subjectId:identity.userId,artifactId:sourcePublication.artifactId,artifactRevision:sourcePublication.artifactRevision,headerDigest:sourcePublication.headerDigest,bodyRefDigest:sourcePublication.protectedBody.refDigest,purpose:fixture.purposeRef,scopeDigest:sourcePublication.materialLineage!.scopeDigest!,sensitivity:sourcePublication.materialLineage!.sensitivity!,evaluatedAt:fixture.at};
  const deniedRequests=[{...predecessorRequest,subjectId:"denied-user"},{...predecessorRequest,predecessorArtifactOrganizationId:"foreign-organization"},{...predecessorRequest,predecessorOccurrenceId:"stale-occurrence"},{...predecessorRequest,artifactId:"absent-artifact"},{...predecessorRequest,bodyRefDigest:"malformed"}],neutralTraces:unknown[][]=[];
  for(const request of deniedRequests){const start=observedEvents.length;await activeObserver?.observe({eventCategory:"access-check",workflowStage:"reload",transitionCategory:"attempted",outcomeCategory:"attempted",roleCategory:"unavailable",occurrenceCategory:"occurrence-1",viewportCategory:"not-applicable",latencyBucket:"not-measured",replayRecoveryCategory:"none",failureCategory:"none",protectedLoadCategory:"attempted"});const result=await composition.productArtifactAccess.readHistoricalPredecessor(request);assert.notEqual(result.outcome,"accessible");await activeObserver?.observe({eventCategory:"access-check",workflowStage:"reload",transitionCategory:"completed",outcomeCategory:"access-unavailable",roleCategory:"unavailable",occurrenceCategory:"occurrence-1",viewportCategory:"not-applicable",latencyBucket:"not-measured",replayRecoveryCategory:"none",failureCategory:"access",protectedLoadCategory:"unavailable"});neutralTraces.push(observedEvents.slice(start).map(({sequence,correlation,...event})=>event));}assert.ok(neutralTraces.every(value=>JSON.stringify(value)===JSON.stringify(neutralTraces[0])));
  const accessObserver=activeObserver,revokedGrant:ScopedAuthorityGrant={authorityRef:`leadership-conversation:${fixture.actorId}`,policyRef:"leadership-conversation-development:v1",organizationId:identity.organizationId,subjectId:identity.userId,scope:{organizationId:identity.organizationId,type:"organization",id:identity.organizationId},operations:["product-artifact:prepare-again"],sensitivity:["standard"],relationship:"direct",status:"revoked",validFrom:"2026-01-01T00:00:00.000Z"},revokedComposition=await validationComposition(locations,lineageFixtureRoot,[revokedGrant]);activeObserver=accessObserver;const revokedStart=observedEvents.length;await activeObserver?.observe({eventCategory:"access-check",workflowStage:"reload",transitionCategory:"attempted",outcomeCategory:"attempted",roleCategory:"unavailable",occurrenceCategory:"occurrence-1",viewportCategory:"not-applicable",latencyBucket:"not-measured",replayRecoveryCategory:"none",failureCategory:"none",protectedLoadCategory:"attempted"});assert.equal((await revokedComposition.productArtifactAccess.readHistoricalPredecessor(predecessorRequest)).outcome,"withheld");await activeObserver?.observe({eventCategory:"access-check",workflowStage:"reload",transitionCategory:"completed",outcomeCategory:"access-unavailable",roleCategory:"unavailable",occurrenceCategory:"occurrence-1",viewportCategory:"not-applicable",latencyBucket:"not-measured",replayRecoveryCategory:"none",failureCategory:"access",protectedLoadCategory:"unavailable"});neutralTraces.push(observedEvents.slice(revokedStart).map(({sequence,correlation,...event})=>event));assert.ok(neutralTraces.every(value=>JSON.stringify(value)===JSON.stringify(neutralTraces[0])));
  const beforeClosureReplay=await workflow.read(fixture.organizationId);await composition.completeCycle1Closure(closureInput);const afterClosureReplay=await workflow.read(fixture.organizationId);assert.deepEqual(afterClosureReplay,beforeClosureReplay);
  await assert.rejects(()=>composition.completeCycle1Closure({...closureInput,expectedWorkflowRevision:beforeClosureReplay.revision,authorizedProjectionDigest:"f".repeat(64)}),error=>error instanceof ProductWorkflowIncompatibleIdempotencyReplayError&&error.code==="incompatible_idempotency_replay");const afterIncompatible=await workflow.read(fixture.organizationId);assert.deepEqual(afterIncompatible,beforeClosureReplay);
...
async function worker(role: string, root: string, ownerIssuedQuestionId:string, lineageFixtureRoot: string, expectedSeedDigest: string, encodedA?: string, encodedB?: string): Promise<WorkerResult> {
  ...
  const result=role==="prepare-and-freeze"?await processA(root,lineageFixtureRoot):role==="capture-and-review"?await processB(root,encodedA!):role==="route-actual-owners-and-prepare-again"?await processC(root,lineageFixtureRoot,expectedSeedDigest,encodedA!,encodedB!):role==="operation-linked-ambiguity"?await processC(root,lineageFixtureRoot,expectedSeedDigest,encodedA!,encodedB!,true):role==="reload-direct-evidence-successor"?await processD(root,lineageFixtureRoot,encodedA!):null;
  if(!result)throw new Error("unknown process role");return{...result,observations:observedEvents};
}
...
async function execute(root: string, role: string, lineageFixtureRoot: string | null, expectedSeedDigest: string | null, ...handoffs: SafeHandoff[]): Promise<WorkerResult> {
  ...
  const { stdout, stderr } = await runFile(process.execPath, args, { cwd: process.cwd(), env: { PATH: process.env.PATH ?? "", NODE_PATH: process.env.NODE_PATH ?? "", NODE_ENV: "test", TZ: "UTC", LANG: "C", TMPDIR: tmpdir(),AR3_OBSERVER_MODE:process.env.AR3_OBSERVER_MODE??"enabled",...(process.env.DISCOVERY_ALPHA_TELEMETRY_ROOT?{DISCOVERY_ALPHA_TELEMETRY_ROOT:process.env.DISCOVERY_ALPHA_TELEMETRY_ROOT}:{}),...(process.env.DISCOVERY_ALPHA_TELEMETRY_ACTIVE_KEY_VERSION?{DISCOVERY_ALPHA_TELEMETRY_ACTIVE_KEY_VERSION:process.env.DISCOVERY_ALPHA_TELEMETRY_ACTIVE_KEY_VERSION}:{}),...(process.env.DISCOVERY_ALPHA_TELEMETRY_KEY_RING_JSON?{DISCOVERY_ALPHA_TELEMETRY_KEY_RING_JSON:process.env.DISCOVERY_ALPHA_TELEMETRY_KEY_RING_JSON}:{}),...(process.env.DISCOVERY_ENV?{DISCOVERY_ENV:process.env.DISCOVERY_ENV}:{}) }, timeout: 30_000, maxBuffer: 128 * 1024, shell: false });
  assert.equal(stderr, "");
  const parsed = JSON.parse(stdout) as WorkerResult;
  assert.deepEqual(Object.keys(parsed).sort(), ["assertions", "handoff", "observations", "role"]);
...
async function main(): Promise<void> {
  ...
  try {
    ...
    process.env.AR3_OBSERVER_MODE="disabled";const disabled=await execute(root,"reload-direct-evidence-successor",lineageFixtureRoot,null,c.handoff);process.env.AR3_OBSERVER_MODE="throwing";const throwing=await execute(root,"reload-direct-evidence-successor",lineageFixtureRoot,null,c.handoff);delete process.env.AR3_OBSERVER_MODE;assert.deepEqual(disabled.handoff,d.handoff);assert.deepEqual(throwing.handoff,d.handoff);assert.equal(disabled.observations?.length,0);assert.equal(throwing.observations?.length,0);checks+=4;
    const occurrenceActions=await readFile("app/product-alpha/leadership-conversation/actions.ts","utf8"),experience=await readFile("components/product-alpha/leadership-conversation/LeadershipConversationExperience.tsx","utf8"),telemetryOwner=await readFile("lib/telemetry/alphaProductTelemetryOwner.ts","utf8");assert.match(occurrenceActions,/acceptOccurrence1EvidenceAction[\s\S]*server\.review[\s\S]*server\.routeApproved/);assert.match(experience,/Accept as Evidence/);assert.match(telemetryOwner,/return"repository-unavailable"/);assert.doesNotMatch(telemetryOwner,/throw new Error/);checks+=4;
