...
export async function addFounderGovernedContextAction(seriesAddress:string,_previous:FounderAddContextActionState,data:FormData):Promise<FounderAddContextActionState>{
 ...
 let input;try{input=await parseFounderAddGovernedContextForm(data);}catch(error){return{status:"error",sourceCount:null,destination:null,message:error instanceof Error?error.message:"Check the selected files and purposes."};}
