...
export default async function MeetingHome({params}:{params:Promise<{seriesAddress:string}>}){
 ...
 const analysisResponse=await server.analyzeSourceScopedForDevelopment({userId,organizationId:meeting.organizationId,questionId:meeting.questionId,seriesId:meeting.seriesId,occurrenceId:meeting.occurrenceId}),analysis=analysisResponse.result.status==="eligible"?analysisResponse.result.candidate:undefined;
