...
export async function dispositionOccurrence1CarryForwardAction(input:{proposalId:string;disposition:"accept"|"correct"|"reject"|"needs-information";correctedSummary?:string;seriesAddress?:string}):Promise<LeadershipConversationWorkspaceV1>{
  observeJourney("review","attempted","attempted");
  const{server,identity,seriesId,purposeRef,close}=await reviewedCarryForwardContext(input.seriesAddress);try{let current=await server.workspace({...identity,seriesId}),proposal=current.proposals.find(item=>item.proposalId===input.proposalId);
  ...
  current=await server.workspace({...identity,seriesId});
  if(disposition.startsWith("approved"))await server.ensureReviewedCarryForwardRoute({...identity,seriesId,proposalId:proposal.proposalId,purposeRef,expectedWorkflowRevision:current.workflowRevision,idempotencyKey:`occurrence-1-carry-forward-route:${proposal.proposalId}`});
  else{current=await server.workspace({...identity,seriesId});const reviewed=current.proposals.filter(item=>item.reviewedCarryForward),allReviewed=reviewed.every(item=>current.dispositions.some(value=>value.proposalId===item.proposalId)),allTerminal=current.dispositions.filter(item=>item.disposition.startsWith("approved")&&reviewed.some(value=>value.proposalId===item.proposalId)).every(item=>current.canonicalRoutingReceipts.some(value=>value.proposalId===item.proposalId&&value.dispositionReceiptId===item.dispositionReceiptId)||current.reviewedCarryForwardNonpromotions.some(value=>value.proposalId===item.proposalId&&value.dispositionReceiptId===item.dispositionReceiptId));if(allReviewed&&allTerminal&&!current.reviewedCarryForwardCompletion)await server.ensureReviewedCarryForwardCompletion({...identity,seriesId});}
  const workspace=await server.workspace({...identity,seriesId});observeJourney("review","completed",input.disposition==="needs-information"?"expected-abstention":"success");return workspace;}finally{await close();}
}
