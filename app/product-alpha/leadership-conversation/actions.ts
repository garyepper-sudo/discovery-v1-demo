...
export async function startReviewedCarryForwardFromMeetingPackAction(input:{seriesAddress:string}):Promise<{status:"review-available";reviewAvailable:true}>{
  if(!exactActionRecord(input,["seriesAddress"])||typeof input.seriesAddress!=="string")throw new Error("Reviewed Carry-Forward request is invalid.");
  const context=await currentMeetingPackContext(input.seriesAddress);
  try{await context.server.beginReviewedCarryForwardFromMeetingPack({...context.identity,seriesId:context.seriesId});return{status:"review-available",reviewAvailable:true};}
...
export async function dispositionOccurrence1CarryForwardAction(input:{proposalId:string;disposition:"accept"|"correct"|"reject"|"needs-information";correctedSummary?:string;seriesAddress?:string}):Promise<LeadershipConversationWorkspaceV1>{
  observeJourney("review","attempted","attempted");
  const{server,identity,purposeRef,close}=await reviewedCarryForwardContext(input.seriesAddress);try{let current=await server.workspace(identity),proposal=current.proposals.find(item=>item.proposalId===input.proposalId);
  ...
  if(disposition.startsWith("approved"))await server.ensureReviewedCarryForwardRoute({...identity,proposalId:proposal.proposalId,purposeRef,expectedWorkflowRevision:current.workflowRevision,idempotencyKey:`occurrence-1-carry-forward-route:${proposal.proposalId}`});
  ...
  const workspace=await server.workspace(identity);observeJourney("review","completed",input.disposition==="needs-information"?"expected-abstention":"success");return workspace;}finally{await close();}
}

export async function resumeOccurrence1CarryForwardRouteAction(input:{proposalId:string;seriesAddress?:string}):Promise<LeadershipConversationWorkspaceV1>{const{server,identity,purposeRef,close}=await reviewedCarryForwardContext(input.seriesAddress);try{const current=await server.workspace(identity),proposal=current.proposals.find(item=>item.proposalId===input.proposalId&&item.reviewedCarryForward),disposition=current.dispositions.find(item=>item.proposalId===input.proposalId);if(!proposal||!disposition?.disposition.startsWith("approved"))throw new Error("Reviewed carry-forward route is unavailable.");return await server.ensureReviewedCarryForwardRoute({...identity,proposalId:proposal.proposalId,purposeRef,expectedWorkflowRevision:current.workflowRevision,idempotencyKey:`occurrence-1-carry-forward-route:${proposal.proposalId}`});}finally{await close();}}
export async function resumeOccurrence1CarryForwardCompletionAction(seriesAddress?:string):Promise<LeadershipConversationWorkspaceV1>{const{server,identity,close}=await reviewedCarryForwardContext(seriesAddress);try{return await server.ensureReviewedCarryForwardCompletion(identity);}finally{await close();}}
