...
export async function buildMeetingPackAction(...runtimeArguments:unknown[]){const seriesAddress=parseBuildMeetingPackArguments(runtimeArguments),{server,identity,seriesId}=await currentMeetingPackContext(seriesAddress);return server.buildMeetingPack({...identity,seriesId});}
