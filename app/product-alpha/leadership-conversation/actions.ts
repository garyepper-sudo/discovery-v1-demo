...
export async function buildMeetingPackAction(...runtimeArguments:unknown[]){const seriesAddress=parseBuildMeetingPackArguments(runtimeArguments),context=await currentMeetingPackContext(seriesAddress);try{await assertMeetingPreparationOpen(context);const{server,identity,seriesId}=context;return await server.buildMeetingPack({...identity,seriesId});}finally{await context.close();}}
