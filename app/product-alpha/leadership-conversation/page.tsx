...
export default async function LeadershipConversationPage() {
  ...
  if (workspace.futurePreparationLink) {
    ...
    return <main className={styles.page}><header className={styles.hero}><p className={styles.eyebrow}>Leadership conversation · Occurrence 2</p><h1>{nextPrepare.meeting.title}</h1><p>{nextPrepare.meeting.purpose}</p><p role="status">Occurrence 2 is prepared. The next meeting has not started.</p></header><section className={styles.workflow} aria-label="Occurrence 2 progress"><div className={styles.workflowSummary}><div><p className={styles.workflowLabel}>Current stage</p><h2>Meeting brief</h2></div><p><strong>Next:</strong> Review this brief before beginning the next meeting.</p></div></section><section className={`${styles.card} ${styles.currentCard}`}><p className={styles.badge}>Prepared from reviewed material only</p><h2>Prepare Again</h2><p>Only the reviewed closure and current authorized evidence shaped this preparation. Private Working and unreviewed meeting content did not carry forward.</p><LeadershipConversationPrepare prepare={nextPrepare} valueLayer={compileChiefOfStaffValueLayerV1(nextPrepare)} /></section></main>;
  }
