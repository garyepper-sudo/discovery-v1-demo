export type MaterialAcquisitionEstimate<T> =
  | { state: "available"; value: T; sourceRef: string; qualification: string; maturity: "synthetic" | "fixture-backed" | "owner-provided" | "outcome-calibrated" }
...
export type MaterialAcquisitionEligibility = {
...
export type MaterialAcquisitionCandidate = {
  ...
  eligibility: MaterialAcquisitionEligibility;
  ...
  expectedEvidenceLineage: { sourceKind: string; sourceScopeRef: string; admissionRequired: true } | null;
};

export type MaterialAcquisitionCandidateEnvelope = {
  schemaVersion: "1";
  envelopeId: string;
  organizationId: string;
  actionOwner: { ownerRef: string; contractVersion: string; authorityRef: string };
  candidate: MaterialAcquisitionCandidate;
  question: { questionId: string; revision: number };
  unknown: { unknownId: string; unknownVersionRef: string };
  ...
  requiredSourceAccess: Array<{ sourceScopeRef: string; authorizationRef: string | null; state: "authorized" | "not-required" | "withheld" | "unavailable" }>;
  privacyConstraints: string[];
  humanBurden: MaterialAcquisitionEstimate<"low" | "moderate" | "high">;
...
export type MaterialInformationAcquisitionInput = {
  ...
  questionId: string;
  understandingRevisionRef: string;
  materialUncertainty: { unknownId: string; unknownVersionRef: string; status: "open" | "targeted"; investigationOpportunityRef: string | null };
  purpose: "improve-understanding" | "resolve-objective-ambiguity" | "resolve-optimization-ambiguity" | "improve-recommendation-eligibility" | "evaluate-decision" | "await-learning";
...
export type MaterialAcquisitionExplanation = { uncertaintyRef: string; rationale: string; alternativeCandidateIds: string[]; limitations: string[]; stoppingCondition: string | null };
export type MaterialInformationAcquisitionResult =
  | { kind: "selected-action"; selectionId: string; selected: MaterialAcquisitionCandidate; alternatives: MaterialAcquisitionCandidate[]; explanation: MaterialAcquisitionExplanation }
  | { kind: "material-tie"; selectionId: string; candidates: MaterialAcquisitionCandidate[]; discriminatingQuestion: string | null; explanation: MaterialAcquisitionExplanation }
  | { kind: "stop"; selectionId: string; reason: "understanding-sufficient" | "insufficient-net-value" | "awaiting-outcome" | "budget-exhausted" | "remaining-actions-unreliable" | "user-declined"; explanation: MaterialAcquisitionExplanation }
  | { kind: "abstain"; selectionId: string; reason: "no-authorized-action" | "governance-blocked" | "missing-material-input" | "incomparable-actions" | "selector-unsupported"; explanation: MaterialAcquisitionExplanation };

export type MaterialAcquisitionCalibrationManifest = {
  manifestVersion: "1";
  registeredBeforeSelection: true;
  cases: Array<{ caseId: string; classification: "live" | "controlled-synthetic"; organizationRef: string; questionRevisionRef: string; unknownVersionRef: string; comparatorSource: string; candidateIds: string[]; expectedHardGate: string; holdout: boolean; missingOutcomeData: string[] }>;
  metrics: string[];
