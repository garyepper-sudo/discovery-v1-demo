...
export function normalizeLeadershipConversationArtifactStore(
  ...
  return {
    ...
    registeredMeetingPreparationScopes: store.registeredMeetingPreparationScopes ?? [],
...
type WorkflowEntryRow={collection:string;ordinal:number|string;payload_text:string};
const hostedCollections=["contexts","preparedWorkProducts","frozenSnapshots","registeredMeetingPreparationScopes","preparedWorkPublications","frozenSnapshotPublications","privateWorkingContributionFreezeIntents","privateWorkingContributionPublications","privateWorkingContributionReceipts","privateWorkingContributionCaptures","privateWorkingContributionCaptureReceipts","sharedMeetingPerspectives","meetingPackPrivateNotePublications","meetingPackPublications","publicationReceipts","whatChangedPublications","cycle1ClosureCompletions","reviewedCarryForwardNonpromotions","reviewedCarryForwardCompletions","uploadReceipts","proposals","dispositions","canonicalRoutingReceipts","routingLinks","changeLinks","futurePreparationLinks","productMaterializations","productMaterializationReceipts","historicalCheckpointLifecycleLinks","historicalCheckpointLifecycleLinkReceipts","events"] as const;
/** Hosted adapter retains the existing store contract while PostgreSQL owns the
 * append-only records, replay keys, and revision head. */
export class PostgresProductWorkflowArtifactRepository implements ProductWorkflowArtifactRepository {
  constructor(private readonly sql:Sql<Record<string,unknown>>) {}
  private empty(organizationId:string){return empty(organizationId);}
  async read(organizationId:string):Promise<ProductWorkflowStoreSnapshot>{const head=await this.sql<{repository_revision:string;store_digest:string}[]>`SELECT repository_revision,store_digest FROM chief_workflow_heads WHERE organization_id=${organizationId}`;if(!head.length)return{store:this.empty(organizationId),revision:null};if(head.length!==1)throw new Error("Product Workflow store integrity failed.");const rows=await this.sql<WorkflowEntryRow[]>`SELECT collection,ordinal,payload_text FROM chief_workflow_entries WHERE organization_id=${organizationId} ORDER BY collection,ordinal`;const replays=await this.sql<{key_digest:string;request_fingerprint:string;record_ref:string}[]>`SELECT key_digest,request_fingerprint,record_ref FROM chief_workflow_replays WHERE organization_id=${organizationId} ORDER BY ordinal`;const store=this.empty(organizationId) as any;for(const row of rows){if(!(hostedCollections as readonly string[]).includes(row.collection))throw new Error("Product Workflow store integrity failed.");store[row.collection].push(JSON.parse(row.payload_text));}store.idempotency=replays.map(value=>({keyDigest:value.key_digest,requestFingerprint:value.request_fingerprint,recordRef:value.record_ref}));store.storeDigest=head[0].store_digest;const normalized=normalizeLeadershipConversationArtifactStore(store);assertNoLegacyCombinedBodies(normalized);assertGovernedScopeLineage(normalized);if(leadershipDigest(bytes(normalized))!==head[0].repository_revision)throw new Error("Product Workflow store integrity failed.");return{store:normalized,revision:head[0].repository_revision};}
  async inspect(organizationId:string){return this.read(organizationId);}
  async registerMeetingPreparationScope(input:import("./contracts").RegisteredMeetingPreparationScopeV1&{expectedRevision:string|null}){const current=await this.read(input.organizationId);if(current.revision!==input.expectedRevision)throw new ProductWorkflowRevisionConflictError();const store=current.store as LeadershipConversationArtifactStoreV1&{registeredMeetingPreparationScopes?:import("./contracts").RegisteredMeetingPreparationScopeV1[]},existing=(store.registeredMeetingPreparationScopes??[]).find(value=>value.scopeId===input.scopeId);if(existing){if(existing.scopeDigest!==input.scopeDigest)throw new ProductWorkflowIncompatibleIdempotencyReplayError();return{scope:existing,committed:false,revision:current.revision};}const next=structuredClone(store);next.registeredMeetingPreparationScopes=[...(next.registeredMeetingPreparationScopes??[]),input].sort((left,right)=>left.scopeId.localeCompare(right.scopeId));const committed=await this.replace(input.organizationId,next,current.revision);return{scope:input,committed:true,revision:committed.revision};}
  async readOccurrence(input:ProductWorkflowOccurrenceIdentity & {idempotencyKeyDigests?:readonly string[];proposalKinds?:readonly ProposalKind[]}):Promise<ProductWorkflowOccurrenceSlice>{const snapshot=await this.read(input.organizationId),store=structuredClone(snapshot.store);if(input.proposalKinds){const allowed=new Set(input.proposalKinds);store.proposals=store.proposals.filter(value=>allowed.has(value.kind));const ids=new Set(store.proposals.map(value=>value.proposalId));store.dispositions=store.dispositions.filter(value=>ids.has(value.proposalId));store.canonicalRoutingReceipts=store.canonicalRoutingReceipts.filter(value=>ids.has(value.proposalId));store.routingLinks=store.routingLinks.filter(value=>ids.has(value.proposalId));}return{contractVersion:"1",identity:structuredClone(input),storeRevision:snapshot.revision,storeIntegrityRef:leadershipDigest(leadershipStableSerialize({organizationId:input.organizationId,revision:snapshot.revision})),store,internalWholeFileReads:0,internalWholeFileParses:0};}
  async mutateOccurrence(input:ProductWorkflowOccurrenceIdentity&{expectedRevision:string|null;idempotencyKeyDigest:string;requestFingerprint:string},build:(slice:ProductWorkflowOccurrenceSlice,sequence:{eventSequence:number;previousEventId:string|null})=>ProductWorkflowOccurrenceMutation){const current=await this.read(input.organizationId),prior=current.store.idempotency.find(value=>value.keyDigest===input.idempotencyKeyDigest);if(prior){if(prior.requestFingerprint!==input.requestFingerprint)throw new ProductWorkflowIncompatibleIdempotencyReplayError();return{slice:await this.readOccurrence(input),committed:false};}if(current.revision!==input.expectedRevision)throw new ProductWorkflowRevisionConflictError();const slice=await this.readOccurrence({...input,idempotencyKeyDigests:[input.idempotencyKeyDigest]}),events=current.store.events.filter(value=>value.questionId===input.questionId),mutation=build(slice,{eventSequence:events.length+1,previousEventId:events.at(-1)?.eventId??null}),next=structuredClone(current.store);next.cycle1ClosureCompletions??=[];next.reviewedCarryForwardCompletions??=[];next.cycle1ClosureCompletions.push(...(mutation.cycle1ClosureCompletions??[]));next.reviewedCarryForwardCompletions.push(...(mutation.reviewedCarryForwardCompletions??[]));next.events.push(...(mutation.events??[]));next.idempotency.push(...(mutation.idempotency??[]));await this.replace(input.organizationId,next,current.revision);return{slice:await this.readOccurrence(input),committed:true};}
  async readHistoricalCheckpointLifecycle(input:ProductWorkflowLifecycleIdentity&{idempotencyKeyDigests?:readonly string[]}):Promise<ProductWorkflowLifecycleSlice>{const snapshot=await this.read(input.organizationId),checkpoint=(snapshot.store.frozenSnapshotPublications??[]).find(value=>value.organizationId===input.organizationId&&value.productQuestionId===input.questionId&&value.productWorkflowId===`leadership-conversation:${input.conversationId}`&&value.artifactId===input.checkpointId&&value.artifactRevision===input.checkpointRevision)??null,links=(snapshot.store.historicalCheckpointLifecycleLinks??[]).filter(value=>value.organizationId===input.organizationId&&value.productQuestionId===input.questionId&&value.productWorkflowId===`leadership-conversation:${input.conversationId}`&&value.checkpointId===input.checkpointId&&value.checkpointRevision===input.checkpointRevision),linkIds=new Set(links.map(value=>value.linkId)),receipts=(snapshot.store.historicalCheckpointLifecycleLinkReceipts??[]).filter(value=>linkIds.has(value.linkId)),keys=new Set(input.idempotencyKeyDigests??[]),idempotency=snapshot.store.idempotency.filter(value=>!keys.size||keys.has(value.keyDigest));return{contractVersion:"1",identity:structuredClone(input),storeRevision:snapshot.revision,checkpointPublication:checkpoint?structuredClone(checkpoint):null,links:structuredClone(links),receipts:structuredClone(receipts),idempotency:structuredClone(idempotency)};}
  async mutateHistoricalCheckpointLifecycle(input:ProductWorkflowLifecycleIdentity&{expectedRevision:string|null;idempotencyKeyDigest:string;requestFingerprint:string},build:(slice:ProductWorkflowLifecycleSlice,sequence:{eventSequence:number;previousEventId:string|null})=>ProductWorkflowLifecycleMutation){const current=await this.read(input.organizationId),slice=await this.readHistoricalCheckpointLifecycle({...input,idempotencyKeyDigests:[input.idempotencyKeyDigest]}),prior=slice.idempotency.find(value=>value.keyDigest===input.idempotencyKeyDigest);if(prior){if(prior.requestFingerprint!==input.requestFingerprint)throw new ProductWorkflowIncompatibleIdempotencyReplayError();return{slice,committed:false};}if(current.revision!==input.expectedRevision)throw new ProductWorkflowRevisionConflictError();const events=current.store.events.filter(value=>value.questionId===input.questionId),mutation=build(slice,{eventSequence:events.length+1,previousEventId:events.at(-1)?.eventId??null}),next=structuredClone(current.store);next.historicalCheckpointLifecycleLinks??=[];next.historicalCheckpointLifecycleLinkReceipts??=[];if(mutation.links.some(value=>next.historicalCheckpointLifecycleLinks!.some(existing=>existing.linkId===value.linkId))||mutation.receipts.some(value=>next.historicalCheckpointLifecycleLinkReceipts!.some(existing=>existing.receiptId===value.receiptId))||mutation.events.some(value=>next.events.some(existing=>existing.eventId===value.eventId)))throw new Error("Historical checkpoint lifecycle identity collision.");next.historicalCheckpointLifecycleLinks.push(...mutation.links);next.historicalCheckpointLifecycleLinkReceipts.push(...mutation.receipts);next.events.push(...mutation.events);next.idempotency.push(...mutation.idempotency);await this.replace(input.organizationId,next,current.revision);return{slice:await this.readHistoricalCheckpointLifecycle(input),committed:true};}
  async replace(organizationId:string,store:LeadershipConversationArtifactStoreV1,expectedRevision:string|null){let next=normalizeLeadershipConversationArtifactStore(store);if(next.organizationId!==organizationId)throw new Error("Product Workflow store organization mismatch.");assertNoLegacyCombinedBodies(next);assertGovernedScopeLineage(next);const raw=bytes(next),revision=leadershipDigest(raw),persisted=JSON.parse(new TextDecoder().decode(raw)) as LeadershipConversationArtifactStoreV1;next=normalizeLeadershipConversationArtifactStore(persisted);const last=[...next.events].sort((left,right)=>right.eventSequence-left.eventSequence||right.eventId.localeCompare(left.eventId))[0];return this.sql.begin("isolation level serializable",async tx=>{await tx`SELECT pg_advisory_xact_lock(hashtextextended(${`chief-workflow:${organizationId}`},0))`;const current=await tx<{repository_revision:string}[]>`SELECT repository_revision FROM chief_workflow_heads WHERE organization_id=${organizationId} FOR UPDATE`;const actual=current[0]?.repository_revision??null;if(actual!==expectedRevision)throw new ProductWorkflowRevisionConflictError();if(!current.length)await tx`INSERT INTO chief_workflow_heads (organization_id,repository_revision,store_digest,last_event_sequence,last_event_id,contract_version) VALUES (${organizationId},${revision},${next.storeDigest},${last?.eventSequence??0},${last?.eventId??null},'1')`;else await tx`UPDATE chief_workflow_heads SET repository_revision=${revision},store_digest=${next.storeDigest},last_event_sequence=${last?.eventSequence??0},last_event_id=${last?.eventId??null} WHERE organization_id=${organizationId}`;for(const collection of hostedCollections){const values=(next as any)[collection]??[];for(const [ordinal,value] of values.entries()){const prior=await tx<{payload_text:string}[]>`SELECT payload_text FROM chief_workflow_entries WHERE organization_id=${organizationId} AND collection=${collection} AND ordinal=${ordinal}`;const payload=JSON.stringify(value);if(prior[0]&&prior[0].payload_text!==payload)throw new Error("Product Workflow immutable record changed.");if(!prior.length)await tx`INSERT INTO chief_workflow_entries (organization_id,collection,ordinal,payload_text,question_id,series_id,conversation_id,checkpoint_id,checkpoint_revision,event_sequence,body_ref_digests) VALUES (${organizationId},${collection},${ordinal},${payload},${(value as any).questionId??(value as any).productQuestionId??null},${(value as any).seriesId??null},${(value as any).conversationId??null},${(value as any).checkpointId??null},${(value as any).checkpointRevision??null},${collection==='events'?(value as any).eventSequence??null:null},${[]})`;}}for(const value of next.idempotency){const prior=await tx<{request_fingerprint:string;record_ref:string}[]>`SELECT request_fingerprint,record_ref FROM chief_workflow_replays WHERE organization_id=${organizationId} AND key_digest=${value.keyDigest}`;if(prior[0]&&(prior[0].request_fingerprint!==value.requestFingerprint||prior[0].record_ref!==value.recordRef))throw new ProductWorkflowIncompatibleIdempotencyReplayError();if(!prior.length)await tx`INSERT INTO chief_workflow_replays (organization_id,key_digest,request_fingerprint,record_ref,ordinal) VALUES (${organizationId},${value.keyDigest},${value.requestFingerprint},${value.recordRef},${next.idempotency.indexOf(value)})`;}return{store:next,revision};});}
  async resetDevelopmentFixture(_organizationId:string):Promise<boolean>{throw new Error("Product Workflow reset is unavailable for hosted storage.");}
}
