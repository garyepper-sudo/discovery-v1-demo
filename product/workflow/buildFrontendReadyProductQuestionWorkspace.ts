...
export function buildFrontendReadyProductQuestionWorkspace(input: ProductQuestionWorkspaceV2ProjectionInput): ProductQuestionWorkspaceV2 {
  ...
  const stages: ProductWorkflowStage[] = [
    ...
    stage({ id: "operation", status: decision?.eventType === "improvement-authorized" ? operationOwnerAvailable ? "available" : "blocked" : outcome ? "complete" : "not-started", sourceRefs: decision ? [decision.operationId] : [], summary: operationOwnerAvailable ? "A canonical local operation owner is available." : "No canonical local operation executor is implemented for this action.", blockedReason: decision?.eventType === "improvement-authorized" && !operationOwnerAvailable ? "operation-owner-unimplemented" : null, requiredHumanAction: null, nextAction: decision?.eventType === "improvement-authorized" && operationOwnerAvailable ? "execute-existing-local-read-only-operation" : null, auditRefs: decision ? [decision.operationId] : [], unavailable: !operationOwnerAvailable }),
    stage({ id: "outcome", status: outcome ? "complete" : "not-started", sourceRefs: outcome ? [outcome.observationId, outcome.observationSourceRef] : [], summary: outcome ? `Operation outcome state: ${outcome.state}.` : "No canonical operation Outcome has been observed.", blockedReason: outcome ? null : "outcome-unmeasured", requiredHumanAction: null, nextAction: outcome ? "view-lineage" : null, auditRefs: outcome ? [outcome.observationId] : [], unavailable: !outcome }),
