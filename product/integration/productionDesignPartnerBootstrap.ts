...
import { createDurableProductQuestion, buildDurableProductQuestion } from "../questions/questionLifecycle";
...
function inputIsValid(input: ProductionDesignPartnerBootstrapInputV1): void {
  if (process.env.NODE_ENV !== "production" || Object.keys(input).sort().join("\0") !== ["cadenceLabel", "clerkSubject", "contractVersion", "meetingExternalKey", "meetingPurpose", "meetingTitle", "occurredAt", "operationId", "organizationId", "preparationScopeExternalKey", "productQuestion", "productQuestionExternalKey", "role", "sources"].join("\0")) throw new Error("Production bootstrap is unavailable.");
  if (input.contractVersion !== "1" || ![input.clerkSubject, input.organizationId, input.operationId, input.productQuestionExternalKey, input.meetingExternalKey, input.preparationScopeExternalKey].every(exact) || ![input.organizationId, input.operationId, input.productQuestionExternalKey, input.meetingExternalKey, input.preparationScopeExternalKey].every(value => key.test(value))) throw new Error("Production bootstrap input is invalid.");
...
export async function bootstrapProductionDesignPartner(input: ProductionDesignPartnerBootstrapInputV1): Promise<ProductionDesignPartnerBootstrapReceiptV1> {
  ...
  try {
    ...
    const existingRuntime=await infrastructure.runtime.read(input.organizationId);
    if(!existingRuntime) throw new Error("Production bootstrap organization Runtime is unavailable.");

    // ProductQuestion remains Runtime-owned.  CAS retry only accepts the exact
    // deterministic Question already written by this operation.
    const questionId=`product-question:${digest([input.organizationId,input.productQuestionExternalKey]).slice(0,32)}`;
    for(let attempt=0;attempt<3;attempt++){
      const stored=await infrastructure.runtime.read(input.organizationId); if(!stored) throw new Error("Production bootstrap organization Runtime is unavailable.");
      const current=buildDurableProductQuestion({runtime:stored.runtime,questionId});
      if(current){if(current.title!==input.productQuestion) throw new Error("Production bootstrap Product Question conflicts."); break;}
      const next=createDurableProductQuestion({runtime:stored.runtime,questionId,title:input.productQuestion,createdAt:at}).runtime;
      try { await infrastructure.runtime.replace(input.organizationId,new TextEncoder().encode(JSON.stringify(next,null,2)),stored.revision,{requestId:`${input.operationId}:question`,operatorId:participant.participantRef}); break; } catch { if(attempt===2) throw new Error("Production bootstrap Product Question could not be persisted."); }
    }
    const verifiedQuestion=(await infrastructure.runtime.read(input.organizationId))?.runtime;
    if(!verifiedQuestion || !buildDurableProductQuestion({runtime:verifiedQuestion,questionId})) throw new Error("Production bootstrap Product Question is unavailable.");

    await access.activatePolicy({organizationId:input.organizationId,issuerAuthority:issuer,operationId:`${input.operationId}:policy`,occurredAt:at});
    await access.grant({organizationId:input.organizationId,participantRef:participant.participantRef,scope:"organization",issuerAuthority:issuer,operationId:`${input.operationId}:organization`,occurredAt:at});
    ...
    for(const source of input.sources){
      ...
      const stored=await infrastructure.runtime.read(input.organizationId); if(!stored) throw new Error("Production bootstrap Runtime is unavailable.");
      const context=authority({organizationId:input.organizationId,subjectId:participant.participantRef,issuer,at,operation:"source-binding:register-local"});
      let binding; try { binding=await bindings.registerCanonicalLocalSourceBinding({contractVersion:"1",organizationId:input.organizationId,productQuestionId:questionId,sourceType:source.mediaType==="text/markdown"?"markdown-upload":"plain-text-upload",purposeRef:PURPOSE,normalizedContentDigest:normalizedDigest,requestedScopeAssertions:[{relationship:"applies-to",scope:{organizationId:input.organizationId,type:"organization",id:input.organizationId}}],sensitivity:"standard",authorization:context,recordedAt:at,recordedByActorRef:participant.participantRef,idempotencyKey:`${input.operationId}:source:${source.externalKey}:binding`,expectedRuntimeRevision:stored.revision,operation:{requestId:`${input.operationId}:source:${source.externalKey}:binding`,operatorId:participant.participantRef}}); } catch { const resolved=await bindings.resolveCanonicalCurrentSourceBinding({contractVersion:"1",organizationId:input.organizationId,productQuestionId:questionId,sourceType:source.mediaType==="text/markdown"?"markdown-upload":"plain-text-upload",purposeRef:PURPOSE,normalizedContentDigest:normalizedDigest,requestedScopeAssertions:[{relationship:"applies-to",scope:{organizationId:input.organizationId,type:"organization",id:input.organizationId}}],sensitivity:"standard",authorization:authority({organizationId:input.organizationId,subjectId:participant.participantRef,issuer,at,operation:"source-binding:resolve-current"}),resolvedAt:at}); binding={sourceBindingId:resolved.binding.bindingId}; }
      const state=await content.inspectExactWriteState({organizationId:input.organizationId,productQuestionId:questionId,sourceBindingId:binding.sourceBindingId,purposeRef:PURPOSE,normalizedContentDigest:normalizedDigest,exactContentDigest:sourceContentDigest(source.bytes),byteLength:source.bytes.byteLength,authorization:authority({organizationId:input.organizationId,subjectId:participant.participantRef,issuer,at,operation:"source-content:write"})});
      if(state.status==="conflict") throw new Error("Production bootstrap source content conflicts.");
      if(state.status==="incomplete") await content.restoreMissingBody({contractVersion:"1",organizationId:input.organizationId,sourceBindingId:binding.sourceBindingId,sourceContentVersionId:state.version.sourceContentVersionId,purposeRef:PURPOSE,bytes:source.bytes,restoredByActorRef:participant.participantRef,authorization:authority({organizationId:input.organizationId,subjectId:participant.participantRef,issuer,at,operation:"source-content:write"})});
      if(state.status==="absent") await content.write({contractVersion:"1",organizationId:input.organizationId,sourceBindingId:binding.sourceBindingId,purposeRef:PURPOSE,mediaType:source.mediaType,bytes:source.bytes,storedAt:at,storedByActorRef:participant.participantRef,idempotencyKey:`${input.operationId}:source:${source.externalKey}:content`,expectedRepositoryRevision:state.repositoryRevision,authorization:authority({organizationId:input.organizationId,subjectId:participant.participantRef,issuer,at,operation:"source-content:write"})});
      const complete=await content.inspectExactWriteState({organizationId:input.organizationId,productQuestionId:questionId,sourceBindingId:binding.sourceBindingId,purposeRef:PURPOSE,normalizedContentDigest:normalizedDigest,exactContentDigest:sourceContentDigest(source.bytes),byteLength:source.bytes.byteLength,authorization:authority({organizationId:input.organizationId,subjectId:participant.participantRef,issuer,at,operation:"source-content:write"})});
    ...
    const identity=deriveRecurringMeetingOccurrenceIdentity({organizationId:input.organizationId,meetingExternalKey:input.meetingExternalKey}), seriesId=identity.seriesId;
    const scope=completeRegisteredMeetingPreparationScopeV1({contractVersion:"1",scopeId:leadershipId("registered-meeting-preparation-scope",input.organizationId,seriesId,input.preparationScopeExternalKey),organizationId:input.organizationId,questionId,conversationId:identity.conversationId,seriesId,sourceVersions:versions.sort((a,b)=>a.sourceBindingId.localeCompare(b.sourceBindingId)),createdAt:at,createdByUserId:input.clerkSubject,idempotencyKeyDigest:leadershipId("general-recurring-meeting-scope-key",input.organizationId,input.meetingExternalKey),requestFingerprint:digest(input)});
    const before=await infrastructure.workflow.read(input.organizationId); await infrastructure.workflow.registerMeetingPreparationScope({...scope,expectedRevision:before.revision});
    await access.grant({organizationId:input.organizationId,participantRef:participant.participantRef,scope:"meeting-series",meetingSeriesId:seriesId,issuerAuthority:issuer,operationId:`${input.operationId}:series`,occurredAt:at});
