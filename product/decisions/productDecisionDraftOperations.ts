...
import { validateProductArtifactInspectionMetadataV1, type ProductArtifactInspectionMetadataV1 } from "../workflow/productArtifactInspectionMetadataContracts";
...
function validateEvent(value: unknown, runtime: OrganizationRuntime): ProductDecisionDraftRevisionEventV1 | null {
  ...
  if(event.revision.bodyStoredExternally){
    if(!event.revision.protectedBody||!event.revision.bodyStageReceiptDigest||!event.revision.inspectionMetadata)throw new Error("Product Decision Draft body publication is incomplete.");
    validateProductArtifactBodyRefV1(event.revision.protectedBody);
    validateProductArtifactInspectionMetadataV1(event.revision.inspectionMetadata);
    ...
    if(event.revision.inspectionMetadata.organizationId!==event.organizationId||event.revision.inspectionMetadata.semanticOwner!=="product-decision-draft"||event.revision.inspectionMetadata.artifactType!=="product-decision-draft"||event.revision.inspectionMetadata.artifactId!==event.draftId||event.revision.inspectionMetadata.artifactRevision!==event.revision.revisionId||event.revision.inspectionMetadata.productQuestionId!==event.questionId||event.revision.inspectionMetadata.protectedBody.refDigest!==event.revision.protectedBody.refDigest||event.revision.inspectionMetadata.ownerStageReceiptDigest!==event.revision.bodyStageReceiptDigest||!event.revision.inspectionMetadata.creationEnvelopeDigest||!event.revision.inspectionMetadata.materialReferencesDigest||!event.revision.inspectionMetadata.materialLineage)throw new Error("Product Decision Draft inspection metadata is invalid.");
